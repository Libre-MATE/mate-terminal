...
static void skey_data_free(SkeyData *data) {
  g_free(data->seed);
  g_free(data);
}

static gboolean extract_seq_and_seed(const gchar *skey_match, gint *seq,
                                     gchar **seed) {
  gchar *end_ptr = NULL;
  ...
  *seq = strtol(skey_match + strlen(SKEY_PREFIX), &end_ptr, 0);
  ...
  if (end_ptr == NULL || *end_ptr == '\000') return FALSE;
...
static gboolean extract_hash_seq_and_seed(const gchar *otp_match, gint *hash,
                                          gint *seq, gchar **seed) {
  gchar *end_ptr = NULL;
...
static void skey_challenge_response_cb(GtkWidget *dialog, int response_id,
                                       SkeyData *data) {
...
void terminal_skey_do_popup(GtkWindow *window, TerminalScreen *screen,
  ...
  if (!terminal_util_load_builder_resource(
          ...
          "ui/skey-challenge.ui",
          "skey-dialog", &dialog, "skey-entry", &entry, "text-label", &label,
          "skey-ok-button", &ok_button, NULL)) {
    g_free(seed);
    return;
  }
  ...
  g_signal_connect_data(dialog, "response",
                        G_CALLBACK(skey_challenge_response_cb), data,
                        (GClosureNotify)skey_data_free, 0);
