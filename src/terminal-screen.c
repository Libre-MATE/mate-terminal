...
enum { PROFILE_SET, SHOW_POPUP_MENU, MATCH_CLICKED, CLOSE_SCREEN, LAST_SIGNAL };
...
static gboolean terminal_screen_format_title(TerminalScreen *screen,
                                             const char *raw_title,
                                             char **old_cooked_title);
...
static const TerminalRegexPattern url_regex_patterns[] = {
    ...
    {"(?:www|ftp)" HOSTCHARS_CLASS "*\\." HOST PORT URLPATH,
     FLAVOR_DEFAULT_TO_HTTP, PCRE2_CASELESS},
    {"(?:callto:|h323:|sip:)" USERCHARS_CLASS "[" USERCHARS ".]*(?:" PORT
     "/[a-z0-9]+)?\\@" HOST,
     FLAVOR_VOIP_CALL, PCRE2_CASELESS},
    {"(?:mailto:)?" USERCHARS_CLASS "[" USERCHARS ".]*\\@" HOSTCHARS_CLASS
     "+\\." HOST,
     FLAVOR_EMAIL, PCRE2_CASELESS},
...
static char *cwd_of_pid(int pid) {
  ...
  for (i = 0; i < G_N_ELEMENTS(patterns); ++i) {
    ...
    if (len > 0 && buf[0] == '/') return g_strndup(buf, len);
    ...
    if (len <= 0) {
      ...
      if (cwd != NULL) {
        ...
        if (chdir(cwd_file) == 0) {
          ...
          if (chdir(cwd) < 0) g_warning("Could not change working directory.");
        }
        g_free(cwd);
      }

      if (working_dir) return working_dir;
    }
  }
...
static void free_tag_data(TagData *tagdata) { g_slice_free(TagData, tagdata); }

static void terminal_screen_class_enable_menu_bar_accel_notify_cb(
    TerminalApp *app, GParamSpec *pspec, TerminalScreenClass *klass) {
  static gboolean is_enabled = TRUE; /* the binding is enabled by default since
                                        GtkWidgetClass installs it */
  gboolean enable;
...
static TerminalWindow *terminal_screen_get_window(TerminalScreen *screen) {
  GtkWidget *widget = GTK_WIDGET(screen);
  GtkWidget *toplevel;

  toplevel = gtk_widget_get_toplevel(widget);
  if (!gtk_widget_is_toplevel(toplevel)) return NULL;
...
static void terminal_screen_style_updated(GtkWidget *widget) {
  ...
  if (gtk_widget_get_realized(widget)) terminal_screen_change_font(screen);
}
...
static void terminal_screen_init(TerminalScreen *screen) {
  ...
  gtk_target_list_add_table(target_list, target_table,
  ...
  g_signal_connect(screen, "window-title-changed",
                   G_CALLBACK(terminal_screen_window_title_changed), screen);
  g_signal_connect(screen, "icon-title-changed",
                   G_CALLBACK(terminal_screen_icon_title_changed), screen);
  ...
  _TERMINAL_DEBUG_IF(TERMINAL_DEBUG_GEOMETRY) {
    g_signal_connect_after(screen, "size-allocate", G_CALLBACK(size_allocate),
                           NULL);
  }
#endif
}

static void terminal_screen_get_property(GObject *object, guint prop_id,
...
static void terminal_screen_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  TerminalScreen *screen = TERMINAL_SCREEN(object);
...
static void terminal_screen_class_init(TerminalScreenClass *klass) {
  ...
  signals[PROFILE_SET] = g_signal_new(
      I_("profile-set"), G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(TerminalScreenClass, profile_set), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, TERMINAL_TYPE_PROFILE);

  signals[SHOW_POPUP_MENU] = g_signal_new(
      I_("show-popup-menu"), G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(TerminalScreenClass, show_popup_menu),
      NULL, NULL, g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1,
      G_TYPE_POINTER);

  signals[MATCH_CLICKED] = g_signal_new(
      I_("match-clicked"), G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(TerminalScreenClass, match_clicked),
      g_signal_accumulator_true_handled, NULL,
      _terminal_marshal_BOOLEAN__STRING_INT_UINT, G_TYPE_BOOLEAN, 3,
      G_TYPE_STRING, G_TYPE_INT, G_TYPE_UINT);

  signals[CLOSE_SCREEN] = g_signal_new(
      I_("close-screen"), G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(TerminalScreenClass, close_screen), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  g_object_class_install_property(
      object_class, PROP_PROFILE,
      g_param_spec_string("profile", NULL, NULL, NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      object_class, PROP_ICON_TITLE,
      g_param_spec_string("icon-title", NULL, NULL, NULL,
                          G_PARAM_READABLE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      object_class, PROP_ICON_TITLE_SET,
      g_param_spec_boolean("icon-title-set", NULL, NULL, FALSE,
                           G_PARAM_READABLE | G_PARAM_STATIC_NAME |
                               G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      object_class, PROP_OVERRIDE_COMMAND,
      g_param_spec_boxed("override-command", NULL, NULL, G_TYPE_STRV,
                         G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                             G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
  ...
  g_object_class_install_property(
      object_class, PROP_INITIAL_ENVIRONMENT,
      g_param_spec_boxed("initial-environment", NULL, NULL, G_TYPE_STRV,
                         G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                             G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
  ...
  for (i = 0; i < n_url_regexes; ++i) {
    ...
    url_regexes[i] = vte_regex_new_for_match(
        url_regex_patterns[i].pattern, -1,
  ...
  for (i = 0; i < n_skey_regexes; ++i) {
    ...
    skey_regexes[i] = vte_regex_new_for_match(
        skey_regex_patterns[i].pattern, -1,
...
static void terminal_screen_dispose(GObject *object) {
  ...
  g_signal_handlers_disconnect_matched(settings, G_SIGNAL_MATCH_DATA, 0, 0,
                                       NULL, NULL, screen);
...
static void terminal_screen_finalize(GObject *object) {
  ...
  if (priv->bg_image) g_object_unref(priv->bg_image);

  G_OBJECT_CLASS(terminal_screen_parent_class)->finalize(object);
}

static gboolean terminal_screen_image_draw_cb(GtkWidget *widget, cairo_t *cr,
                                              void *userdata) {
  TerminalScreen *screen = TERMINAL_SCREEN(widget);
  TerminalScreenPrivate *priv = screen->priv;
  ...
  child_surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, alloc.width,
                                             alloc.height);
...
TerminalScreen *terminal_screen_new(TerminalProfile *profile,
                                    char **override_command, const char *title,
                                    const char *working_dir, char **child_env,
                                    double zoom) {
  TerminalScreen *screen;
  TerminalScreenPrivate *priv;
  ...
  if (terminal_profile_get_property_boolean(
          profile, TERMINAL_PROFILE_USE_CUSTOM_DEFAULT_SIZE)) {
    vte_terminal_set_size(VTE_TERMINAL(screen),
                          terminal_profile_get_property_int(
                              profile, TERMINAL_PROFILE_DEFAULT_SIZE_COLUMNS),
                          terminal_profile_get_property_int(
                              profile, TERMINAL_PROFILE_DEFAULT_SIZE_ROWS));
  }

  if (title) terminal_screen_set_override_title(screen, title);
  ...
  if (child_env) terminal_screen_set_initial_environment(screen, child_env);
...
const char *terminal_screen_get_raw_title(TerminalScreen *screen) {
  ...
  if (priv->raw_title) return priv->raw_title;
...
const char *terminal_screen_get_title(TerminalScreen *screen) {
  ...
  if (priv->cooked_title == NULL) terminal_screen_cook_title(screen);
...
const char *terminal_screen_get_icon_title(TerminalScreen *screen) {
  ...
  if (priv->cooked_icon_title == NULL) terminal_screen_cook_icon_title(screen);
 ...
 */
static const char *terminal_screen_get_title_format(TerminalScreen *screen) {
  TerminalScreenPrivate *priv = screen->priv;
  static const char *formats[] = {
      "%A" /* TERMINAL_TITLE_REPLACE */, "%D%-%S" /* TERMINAL_TITLE_BEFORE  */,
      "%S%-%D" /* TERMINAL_TITLE_AFTER   */, "%S" /* TERMINAL_TITLE_IGNORE  */
  };

  return formats[terminal_profile_get_property_enum(
      priv->profile, TERMINAL_PROFILE_TITLE_MODE)];
}
 ...
 *
 * Format title according @format, and stores it in
 * <literal>*titleptr</literal>. Always ensures that *titleptr will be non-NULL.
 *
...
static gboolean terminal_screen_format_title(TerminalScreen *screen,
  ...
  if (priv->override_title)
    ...
    static_title = terminal_profile_get_property_string(priv->profile,
                                                        TERMINAL_PROFILE_TITLE);

  // title = g_string_sized_new (strlen (static_title) + strlen (raw_title) + 3
  // + 1);
  ...
  for (arg = format; *arg; arg += 2) {
    ...
    if (!text_to_append || !text_to_append[0]) continue;

    if (add_sep && title->len > 0) g_string_append(title, " - ");
...
static void terminal_screen_cook_title(TerminalScreen *screen) {
  ...
  if (terminal_screen_format_title(screen, priv->raw_title,
                                   &priv->cooked_title))
...
static void terminal_screen_cook_icon_title(TerminalScreen *screen) {
  ...
  if (terminal_screen_format_title(screen, priv->raw_icon_title,
                                   &priv->cooked_icon_title))
    g_object_notify(G_OBJECT(screen), "icon-title");
}

static void terminal_screen_profile_notify_cb(TerminalProfile *profile,
                                              GParamSpec *pspec,
                                              TerminalScreen *screen) {
  TerminalScreenPrivate *priv = screen->priv;
  ...
  if (gtk_widget_get_realized(GTK_WIDGET(screen)) &&
      (!prop_name || prop_name == I_(TERMINAL_PROFILE_USE_SYSTEM_FONT) ||
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_SCROLLBACK_LINES) ||
    ...
    glong lines = terminal_profile_get_property_boolean(
                      profile, TERMINAL_PROFILE_SCROLLBACK_UNLIMITED)
                      ? -1
                      : terminal_profile_get_property_int(
                            profile, TERMINAL_PROFILE_SCROLLBACK_LINES);
    vte_terminal_set_scrollback_lines(vte_terminal, lines);
  }

#ifdef ENABLE_SKEY
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_USE_SKEY)) {
    if (terminal_profile_get_property_boolean(profile,
                                              TERMINAL_PROFILE_USE_SKEY)) {
      guint i;
      ...
      for (i = 0; i < n_skey_regexes; ++i) {
        ...
        tag_data->tag =
            vte_terminal_match_add_regex(vte_terminal, skey_regexes[i], 0);
        vte_terminal_match_set_cursor_type(vte_terminal, tag_data->tag,
                                           SKEY_MATCH_CURSOR);
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_BACKSPACE_BINDING))
    vte_terminal_set_backspace_binding(
        vte_terminal, terminal_profile_get_property_enum(
                          profile, TERMINAL_PROFILE_BACKSPACE_BINDING));
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_USE_URLS)) {
    if (terminal_profile_get_property_boolean(profile,
      ...
      for (i = 0; i < n_url_regexes; ++i) {
        ...
        tag_data->tag =
            vte_terminal_match_add_regex(vte_terminal, url_regexes[i], 0);
        vte_terminal_match_set_cursor_type(vte_terminal, tag_data->tag,
                                           URL_MATCH_CURSOR);
...
static void update_color_scheme(TerminalScreen *screen) {
  ...
  if (!terminal_profile_get_property_boolean(
    ...
    if (!terminal_profile_get_property_boolean(
      ...
      bold_rgba = terminal_profile_get_property_boxed(
          profile, TERMINAL_PROFILE_BOLD_COLOR);

    if (fg_rgba) fg = *fg_rgba;
    if (bg_rgba) bg = *bg_rgba;
  }

  n_colors = G_N_ELEMENTS(colors);
  terminal_profile_get_palette(priv->profile, colors, &n_colors);

  bg_type = terminal_profile_get_property_enum(
      profile, TERMINAL_PROFILE_BACKGROUND_TYPE);
  bg_image_file = terminal_profile_get_property_string(
      profile, TERMINAL_PROFILE_BACKGROUND_IMAGE_FILE);

  if (bg_type == TERMINAL_BACKGROUND_TRANSPARENT)
    bg_alpha = terminal_profile_get_property_double(
        profile, TERMINAL_PROFILE_BACKGROUND_DARKNESS);
  ...
  if (bg_type == TERMINAL_BACKGROUND_IMAGE) {
    if (!priv->bg_image_callback_id)
      priv->bg_image_callback_id = g_signal_connect(
          screen, "draw", G_CALLBACK(terminal_screen_image_draw_cb), NULL);
  ...
  if (bold_rgba) vte_terminal_set_color_bold(VTE_TERMINAL(screen), bold_rgba);
}
...
void terminal_screen_set_font(TerminalScreen *screen) {
  ...
  if (terminal_profile_get_property_boolean(profile,
                                            TERMINAL_PROFILE_USE_SYSTEM_FONT))
...
static void terminal_screen_system_font_notify_cb(TerminalApp *app,
  ...
  if (!gtk_widget_get_realized(GTK_WIDGET(screen))) return;

  if (!terminal_profile_get_property_boolean(priv->profile,
                                             TERMINAL_PROFILE_USE_SYSTEM_FONT))
    return;
...
static void profile_forgotten_callback(TerminalProfile *profile,
                                       TerminalScreen *screen) {
...
void terminal_screen_set_profile(TerminalScreen *screen,
  ...
  if (old_profile) g_object_unref(old_profile);
...
void terminal_screen_set_override_command(TerminalScreen *screen, char **argv) {
  TerminalScreenPrivate *priv;
...
static gboolean get_child_command(TerminalScreen *screen, const char *shell_env,
                                  GSpawnFlags *spawn_flags_p, char ***argv_p,
                                  GError **err) {
  TerminalScreenPrivate *priv = screen->priv;
  TerminalProfile *profile;
  char **argv;
  ...
  *argv_p = argv = NULL;
  ...
  } else if (terminal_profile_get_property_boolean(
                 profile, TERMINAL_PROFILE_USE_CUSTOM_COMMAND)) {
    if (!g_shell_parse_argv(terminal_profile_get_property_string(
...
static char **get_child_environment(TerminalScreen *screen, char **shell) {
  TerminalScreenPrivate *priv = screen->priv;
  ...
  if (env) {
    for (i = 0; env[i]; ++i) {
      v = strchr(env[i], '=');
      if (v)
        g_hash_table_replace(env_table, g_strndup(env[i], v - env[i]),
                             g_strdup(v + 1));
      else
  ...
  g_hash_table_replace(
      env_table, g_strdup("TERM"),
      g_strdup("xterm-256color")); /* FIXME configurable later? */

  /* FIXME: moving the tab between windows, or the window between displays will
   * make the next two invalid... */
#ifdef GDK_WINDOWING_X11
  if (GDK_IS_X11_DISPLAY(display)) {
    g_hash_table_replace(
        env_table, g_strdup("WINDOWID"),
        g_strdup_printf("%ld", GDK_WINDOW_XID(gtk_widget_get_window(window))));
  }
#endif
  g_hash_table_replace(env_table, g_strdup("DISPLAY"),
                       g_strdup(gdk_display_get_name(display)));

  g_settings_schema_source_list_schemas(g_settings_schema_source_get_default(),
                                        TRUE, &list_schemas, NULL);
...
enum { RESPONSE_RELAUNCH, RESPONSE_EDIT_PROFILE };

static void info_bar_response_cb(GtkWidget *info_bar, int response,
                                 TerminalScreen *screen) {
...
static void handle_error_child(TerminalScreen *screen, GError *err) {
  GtkWidget *info_bar;

  info_bar = terminal_info_bar_new(GTK_MESSAGE_ERROR, _("_Profile Preferences"),
                                   RESPONSE_EDIT_PROFILE, _("_Relaunch"),
                                   RESPONSE_RELAUNCH, NULL);
  terminal_info_bar_format_text(
      TERMINAL_INFO_BAR(info_bar),
      _("There was an error creating the child process for this terminal"));
  terminal_info_bar_format_text(TERMINAL_INFO_BAR(info_bar), "%s",
                                err->message);
  g_signal_connect(info_bar, "response", G_CALLBACK(info_bar_response_cb),
                   screen);
...
static void term_spawn_callback(GtkWidget *terminal, GPid pid, GError *error,
                                gpointer user_data) {
...
static gboolean terminal_screen_launch_child_cb(TerminalScreen *screen) {
  TerminalScreenPrivate *priv = screen->priv;
  ...
  vte_terminal_spawn_async(
      terminal, pty_flags, working_dir, argv, env, spawn_flags, NULL, NULL,
      NULL, -1, NULL, (VteTerminalSpawnAsyncCallback)term_spawn_callback, NULL);
...
static void terminal_screen_launch_child_on_idle(TerminalScreen *screen) {
  ...
  if (priv->launch_child_source_id != 0) return;

  _terminal_debug_print(
      TERMINAL_DEBUG_PROCESSES,
      "[screen %p] scheduling launching the child process on idle\n", screen);

  priv->launch_child_source_id =
      g_idle_add((GSourceFunc)terminal_screen_launch_child_cb, screen);
}

static TerminalScreenPopupInfo *terminal_screen_popup_info_new(
    TerminalScreen *screen) {
...
TerminalScreenPopupInfo *terminal_screen_popup_info_ref(
    TerminalScreenPopupInfo *info) {
...
static gboolean terminal_screen_button_press(GtkWidget *widget,
  ...
  matched_string =
      terminal_screen_check_match(screen, (GdkEvent *)event, &matched_flavor);

  if (matched_string != NULL && (event->button == 1 || event->button == 2) &&
      (state & GDK_CONTROL_MASK)) {
    gboolean handled = FALSE;

#ifdef ENABLE_SKEY
    if (matched_flavor != FLAVOR_SKEY ||
        terminal_profile_get_property_boolean(screen->priv->profile,
                                              TERMINAL_PROFILE_USE_SKEY))
#endif
    {
      g_signal_emit(screen, signals[MATCH_CLICKED], 0, matched_string,
                    matched_flavor, state, &handled);
    }
  ...
  if (button_press_event) return button_press_event(widget, event);

  return FALSE;
}

static void terminal_screen_set_dynamic_title(TerminalScreen *screen,
                                              const char *title,
                                              gboolean userset) {
  TerminalScreenPrivate *priv = screen->priv;
  ...
  if ((priv->user_title && !userset) ||
      (priv->raw_title && title && strcmp(priv->raw_title, title) == 0))
    return;
...
static void terminal_screen_set_dynamic_icon_title(TerminalScreen *screen,
  ...
  if ((priv->user_title && !userset) ||
      (priv->icon_title_set && priv->raw_icon_title && icon_title &&
...
char *terminal_screen_get_current_dir_with_fallback(TerminalScreen *screen) {
  ...
  pty = vte_terminal_get_pty(VTE_TERMINAL(screen));
  if (pty == NULL) return g_strdup(priv->initial_working_directory);

  return terminal_screen_get_current_dir(screen);
}

void terminal_screen_set_font_scale(TerminalScreen *screen, double factor) {
  TerminalScreenPrivate *priv = screen->priv;

  g_return_if_fail(TERMINAL_IS_SCREEN(screen));

  if (factor < TERMINAL_SCALE_MINIMUM) factor = TERMINAL_SCALE_MINIMUM;
  if (factor > TERMINAL_SCALE_MAXIMUM) factor = TERMINAL_SCALE_MAXIMUM;
...
static void terminal_screen_window_title_changed(VteTerminal *vte_terminal,
                                                 TerminalScreen *screen) {
  terminal_screen_set_dynamic_title(
      screen, vte_terminal_get_window_title(vte_terminal), FALSE);
}

static void terminal_screen_icon_title_changed(VteTerminal *vte_terminal,
                                               TerminalScreen *screen) {
  terminal_screen_set_dynamic_icon_title(
      screen, vte_terminal_get_icon_title(vte_terminal), FALSE);
}
...
static void terminal_screen_child_exited(VteTerminal *terminal, int status) {
  ...
  action = terminal_profile_get_property_enum(priv->profile,
                                              TERMINAL_PROFILE_EXIT_ACTION);
  ...
  switch (action) {
    ...
    case TERMINAL_EXIT_HOLD: {
      if ((status == 9) && (priv->override_command == NULL)) break;

      GtkWidget *info_bar;

      info_bar = terminal_info_bar_new(GTK_MESSAGE_INFO, _("_Relaunch"),
                                       RESPONSE_RELAUNCH, NULL);
      if (WIFEXITED(status)) {
        terminal_info_bar_format_text(
            TERMINAL_INFO_BAR(info_bar),
            _("The child process exited normally with status %d."),
            WEXITSTATUS(status));
      } else if (WIFSIGNALED(status)) {
        terminal_info_bar_format_text(
            TERMINAL_INFO_BAR(info_bar),
            _("The child process was terminated by signal %d."),
            WTERMSIG(status));
      } else {
      ...
      gtk_box_pack_start(
          GTK_BOX(terminal_screen_container_get_from_screen(screen)), info_bar,
          FALSE, FALSE, 0);
      gtk_info_bar_set_default_response(GTK_INFO_BAR(info_bar),
                                        RESPONSE_RELAUNCH);
      gtk_widget_show(info_bar);
...
void terminal_screen_set_user_title(TerminalScreen *screen, const char *text) {
  TerminalScreenPrivate *priv = screen->priv;
  ...
  if (gtk_targets_include_uri(&selection_data_target, 1)) {
    ...
    switch (info) {
      case TARGET_COLOR: {
         ...
         */
        if (selection_data_length != 8) return;
        ...
        g_object_set(priv->profile, TERMINAL_PROFILE_BACKGROUND_TYPE,
                     TERMINAL_BACKGROUND_SOLID,
                     TERMINAL_PROFILE_USE_THEME_COLORS, FALSE,
                     TERMINAL_PROFILE_BACKGROUND_COLOR, &color, NULL);
      } break;
      ...
      case TARGET_MOZ_URL: {
         ...
         */
        if (selection_data_format != 8 || selection_data_length == 0 ||
            (selection_data_length % 2) != 0)
          return;

        utf8_data =
            g_utf16_to_utf8((const gunichar2 *)selection_data_data,
                            selection_data_length / 2, NULL, NULL, NULL);
        if (!utf8_data) return;
      ...
      case TARGET_NETSCAPE_URL: {
         ...
         * title of the web page.
         */
        if (selection_data_length < 0 || selection_data_format != 8) return;

        utf8_data =
            g_strndup((char *)selection_data_data, selection_data_length);
        newline = strchr(utf8_data, '\n');
        if (newline) *newline = '\0';
      ...
      case TARGET_RESET_BG:
        g_object_set(priv->profile, TERMINAL_PROFILE_BACKGROUND_TYPE,
                     TERMINAL_BACKGROUND_SOLID, NULL);
        break;
      ...
      case TARGET_TAB: {
        ...
        if (!GTK_IS_WIDGET(container)) return;

        moving_screen = terminal_screen_container_get_screen(
            TERMINAL_SCREEN_CONTAINER(container));
        g_return_if_fail(TERMINAL_IS_SCREEN(moving_screen));
        if (!TERMINAL_IS_SCREEN(moving_screen)) return;
        ...
        page_num = gtk_notebook_page_num(GTK_NOTEBOOK(dest_notebook),
                                         GTK_WIDGET(screen));
        terminal_window_move_screen(source_window, dest_window, moving_screen,
                                    page_num + 1);
...
void _terminal_screen_update_scrollbar(TerminalScreen *screen) {
  ...
  switch (terminal_profile_get_property_enum(
      priv->profile, TERMINAL_PROFILE_SCROLLBAR_POSITION)) {
...
void terminal_screen_get_size(TerminalScreen *screen, int *width_chars,
                              int *height_chars) {
  VteTerminal *terminal = VTE_TERMINAL(screen);
...
static char *terminal_screen_check_match(TerminalScreen *screen,
  ...
  for (tags = priv->match_tags; tags != NULL; tags = tags->next) {
    ...
    if (tag_data->tag == tag) {
      if (flavor) *flavor = tag_data->flavor;
      return match;
    }
  }
...
void terminal_screen_save_config(TerminalScreen *screen, GKeyFile *key_file,
                                 const char *group) {
  TerminalScreenPrivate *priv = screen->priv;
  VteTerminal *terminal = VTE_TERMINAL(screen);
  TerminalProfile *profile = priv->profile;
  ...
  g_key_file_set_string(key_file, group,
                        TERMINAL_CONFIG_TERMINAL_PROP_PROFILE_ID, profile_id);

  if (priv->override_command)
    terminal_util_key_file_set_argv(key_file, group,
                                    TERMINAL_CONFIG_TERMINAL_PROP_COMMAND, -1,
                                    priv->override_command);

  if (priv->override_title)
    g_key_file_set_string(key_file, group, TERMINAL_CONFIG_TERMINAL_PROP_TITLE,
                          priv->override_title);
  ...
  if (working_directory)
    terminal_util_key_file_set_string_escape(
        key_file, group, TERMINAL_CONFIG_TERMINAL_PROP_WORKING_DIRECTORY,
        working_directory);
  g_free(working_directory);

  g_key_file_set_double(key_file, group, TERMINAL_CONFIG_TERMINAL_PROP_ZOOM,
                        priv->font_scale);
