...
static GQuark get_quark(void) {
  static GQuark quark = 0;

  if (G_UNLIKELY(!quark)) quark = g_quark_from_static_string("GT:data");

  return quark;
}

#define TERMINAL_SEARCH_DIALOG_GET_PRIVATE(object)                     \
  ((TerminalSearchDialogPrivate *)g_object_get_qdata(G_OBJECT(object), \
                                                     get_quark()))

#define GET_FLAG(widget) \
  gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->widget))
...
static void update_sensitivity(void *unused, GtkWidget *dialog);
static void response_handler(GtkWidget *dialog, gint response_id,
                             gpointer data);
static void terminal_search_dialog_private_destroy(
...
GtkWidget *terminal_search_dialog_new(GtkWindow *parent) {
  ...
  if (!terminal_util_load_builder_resource(
          TERMINAL_RESOURCES_PATH_PREFIX G_DIR_SEPARATOR_S "ui/find-dialog.ui",
          "find-dialog", &dialog, "search-label", &priv->search_label,
          "search-entry", &priv->search_entry, "match-case-checkbutton",
          &priv->match_case_checkbutton, "entire-word-checkbutton",
          &priv->entire_word_checkbutton, "regex-checkbutton",
          &priv->regex_checkbutton, "search-backwards-checkbutton",
          &priv->backwards_checkbutton, "wrap-around-checkbutton",
          &priv->wrap_around_checkbutton, NULL)) {
    g_free(priv);
    return NULL;
  }

  g_object_set_qdata_full(
      G_OBJECT(dialog), get_quark(), priv,
  ...
  g_object_set(G_OBJECT(priv->search_entry), "model", store,
               "entry-text-column", 0, NULL);
  ...
  g_signal_connect(priv->search_text_entry, "changed",
                   G_CALLBACK(update_sensitivity), dialog);
  g_signal_connect(priv->regex_checkbutton, "toggled",
                   G_CALLBACK(update_sensitivity), dialog);
...
static void terminal_search_dialog_private_destroy(
    TerminalSearchDialogPrivate *priv) {
  if (priv->regex) vte_regex_unref(priv->regex);
...
static void update_sensitivity(void *unused, GtkWidget *dialog) {
  TerminalSearchDialogPrivate *priv =
      TERMINAL_SEARCH_DIALOG_GET_PRIVATE(dialog);
...
static void history_entry_insert(GtkListStore *store, const gchar *text) {
  GtkTreeIter iter;

  g_return_if_fail(text != NULL);

  if (g_utf8_strlen(text, -1) <= HISTORY_MIN_ITEM_LEN) return;
  ...
  if (!remove_item(store, text)) clamp_list_store(store, HISTORY_LENGTH - 1);
...
static void response_handler(GtkWidget *dialog, gint response_id,
  ...
  if (*str != '\0') history_entry_insert(priv->store, str);
}
...
TerminalSearchFlags terminal_search_dialog_get_search_flags(GtkWidget *dialog) {
  ...
  if (GET_FLAG(backwards_checkbutton)) flags |= TERMINAL_SEARCH_FLAG_BACKWARDS;
...
VteRegex *terminal_search_dialog_get_regex(GtkWidget *dialog) {
  ...
  if (!GET_FLAG(match_case_checkbutton)) compile_flags |= PCRE2_CASELESS;
  ...
  if (GET_FLAG(entire_word_checkbutton)) {
    ...
    if (old_pattern != text) g_free((char *)old_pattern);
  }
  ...
  if (!priv->regex || priv->regex_compile_flags != compile_flags) {
    ...
    /* TODO Error handling */
    priv->regex = vte_regex_new_for_search(pattern, -1, compile_flags, NULL);
  }

  if (pattern != text) g_free((char *)pattern);
