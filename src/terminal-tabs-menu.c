 ...
 * efficiently.
 */
static guint allocate_tab_id(void) {
  int bit;
  guint b, len;
  guint8 *data;
  guint8 byte, mask;
...
static void free_tab_id(GtkAction *action) {
  const char *name;
  guint id;
  guint8 *data;
  guint b, bit;
...
static void tab_action_activate_cb(GtkToggleAction *action,
...
static void sync_tab_title(TerminalScreen *screen, GParamSpec *pspec,
                           GtkAction *action) {
  const char *title;
...
static void notebook_page_added_cb(GtkNotebook *notebook,
                                   TerminalScreenContainer *container,
                                   guint position, TerminalTabsMenu *menu) {
  TerminalTabsMenuPrivate *priv = menu->priv;
  GtkAction *action;
  ...
  action = g_object_new(GTK_TYPE_RADIO_ACTION, "name", verb, "tooltip",
                        _("Switch to this tab"), NULL);

  sync_tab_title(screen, NULL, action);
  /* make sure the action is alive when handling the signal, see bug #169833 */
  g_signal_connect_object(screen, "notify::title", G_CALLBACK(sync_tab_title),
                          action, 0);
  ...
  g_signal_connect(action, "activate", G_CALLBACK(tab_action_activate_cb),
                   menu);
...
static void notebook_page_removed_cb(GtkNotebook *notebook,
                                     TerminalScreenContainer *container,
                                     guint position, TerminalTabsMenu *menu) {
  TerminalTabsMenuPrivate *priv = menu->priv;
  GtkAction *action;
  ...
  g_signal_handlers_disconnect_by_func(screen, G_CALLBACK(sync_tab_title),
                                       action);
...
static void notebook_page_reordered_cb(GtkNotebook *notebook, GtkBin *bin,
                                       guint position, TerminalTabsMenu *menu) {
  terminal_tabs_menu_update(menu);
}
...
static void notebook_page_switch_cb(GtkNotebook *notebook, GtkWidget *page,
  ...
  action = g_object_get_data(G_OBJECT(screen), DATA_KEY);
  g_signal_handlers_block_by_func(action, G_CALLBACK(tab_action_activate_cb),
                                  menu);
  gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(action), TRUE);
  g_signal_handlers_unblock_by_func(action, G_CALLBACK(tab_action_activate_cb),
                                    menu);
}

static void connect_proxy_cb(GtkActionGroup *action_group, GtkAction *action,
                             GtkWidget *proxy, gpointer dummy) {
...
static void terminal_tabs_menu_set_window(TerminalTabsMenu *menu,
                                          TerminalWindow *window) {
  TerminalTabsMenuPrivate *priv = menu->priv;
  GtkWidget *notebook;
  ...
  priv->anchor_action =
      g_object_new(GTK_TYPE_RADIO_ACTION, "name", "TabsMenuAnchorAction", NULL);
...
static void terminal_tabs_menu_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void terminal_tabs_menu_get_property(GObject *object, guint prop_id,
...
static void terminal_tabs_menu_class_init(TerminalTabsMenuClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("window", NULL, NULL, TERMINAL_TYPE_WINDOW,
...
static void terminal_tabs_menu_clean(TerminalTabsMenu *menu) {
  TerminalTabsMenuPrivate *p = menu->priv;
  GtkUIManager *manager =
      GTK_UI_MANAGER(terminal_window_get_ui_manager(p->window));
...
TerminalTabsMenu *terminal_tabs_menu_new(TerminalWindow *window) {
  return TERMINAL_TABS_MENU(
      g_object_new(TERMINAL_TYPE_TABS_MENU, "window", window, NULL));
}

static void tab_set_action_accelerator(GtkActionGroup *action_group,
                                       GtkAction *action, guint tab_number,
                                       gboolean is_single_tab) {
  if (!is_single_tab && tab_number < TERMINAL_ACCELS_N_TABS_SWITCH) {
    char accel_path[ACCEL_PATH_FORMAT_LENGTH];

    g_snprintf(accel_path, sizeof(accel_path), ACCEL_PATH_FORMAT,
               tab_number + 1);
...
static void terminal_tabs_menu_update(TerminalTabsMenu *menu) {
  ...
  for (l = tabs; l != NULL; l = l->next) {
    ...
    gtk_ui_manager_add_ui(manager, p->ui_id, UI_PATH, verb, verb,
