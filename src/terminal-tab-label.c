...
enum { PROP_0, PROP_SCREEN };

enum { CLOSE_BUTTON_CLICKED, LAST_SIGNAL };
...
static void close_button_clicked_cb(GtkWidget *widget,
                                    TerminalTabLabel *tab_label) {
  g_signal_emit(tab_label, signals[CLOSE_BUTTON_CLICKED], 0);
}

static void sync_tab_label(TerminalScreen *screen, GParamSpec *pspec,
                           GtkWidget *label) {
  GtkWidget *hbox;
  const char *title;
...
static void terminal_tab_label_parent_set(GtkWidget *widget,
                                          GtkWidget *old_parent) {
  void (*parent_set)(GtkWidget *, GtkWidget *) =
      GTK_WIDGET_CLASS(terminal_tab_label_parent_class)->parent_set;

  if (parent_set) parent_set(widget, old_parent);
}

static void terminal_tab_label_init(TerminalTabLabel *tab_label) {
...
static GObject *terminal_tab_label_constructor(
  ...
  object = G_OBJECT_CLASS(terminal_tab_label_parent_class)
  ...
  g_signal_connect(priv->screen, "notify::title", G_CALLBACK(sync_tab_label),
                   label);
...
static void terminal_tab_label_dispose(GObject *object) {
  TerminalTabLabel *tab_label = TERMINAL_TAB_LABEL(object);
  TerminalTabLabelPrivate *priv = tab_label->priv;

  if (priv->screen != NULL) {
    g_signal_handlers_disconnect_by_func(
        priv->screen, G_CALLBACK(sync_tab_label), priv->label);
...
static void terminal_tab_label_finalize(GObject *object) {
  //   TerminalTabLabel *tab_label = TERMINAL_TAB_LABEL (object);

  G_OBJECT_CLASS(terminal_tab_label_parent_class)->finalize(object);
}

static void terminal_tab_label_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  TerminalTabLabel *tab_label = TERMINAL_TAB_LABEL(object);
  TerminalTabLabelPrivate *priv = tab_label->priv;
...
static void terminal_tab_label_class_init(TerminalTabLabelClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("screen", NULL, NULL, TERMINAL_TYPE_SCREEN,
...
GtkWidget *terminal_tab_label_new(TerminalScreen *screen) {
  return g_object_new(TERMINAL_TYPE_TAB_LABEL, "screen", screen, NULL);
}
...
void terminal_tab_label_set_bold(TerminalTabLabel *tab_label, gboolean bold) {
  ...
  if (free_list) pango_attr_list_unref(attr_list);
}
