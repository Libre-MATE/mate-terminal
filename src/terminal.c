...
static char *ay_to_string(GVariant *variant, GError **error) {
  gsize len;
  const char *data;
...
static char **ay_to_strv(GVariant *variant, int *argc) {
  GPtrArray *argv;
...
static GVariant *string_to_ay(const char *string) {
  gsize len;
  char *data;
  ...
  return g_variant_new_from_data(G_VARIANT_TYPE("ay"), data, len, TRUE, g_free,
                                 data);
}
...
static void method_call_cb(GDBusConnection *connection, const char *sender,
  ...
  if (g_strcmp0(method_name, "HandleArguments") == 0) {
    ...
    g_variant_get(parameters, "(@ay@ay@ay@ayi@ay)", &v_wd, &v_display, &v_sid,
                  &v_envv, &initial_workspace, &v_argv);
    ...
    _terminal_debug_print(
        ...
        startup_id ? startup_id : "(null)", initial_workspace);

    options =
        terminal_options_parse(working_directory, display_name, startup_id,
                               envv, TRUE, TRUE, &argc, &argv, &error, NULL);

    if (options != NULL) {
      options->initial_workspace = initial_workspace;

      terminal_app_handle_options(terminal_app_get(), options,
                                  FALSE /* no resume */, &error);
...
static void bus_acquired_cb(GDBusConnection *connection, const char *name,
                            gpointer user_data) {
  ...
  _terminal_debug_print(TERMINAL_DEBUG_FACTORY, "Bus %s acquired\n", name);
  ...
  registration_id = g_dbus_connection_register_object(
      connection, TERMINAL_FACTORY_SERVICE_PATH,
      introspection_data->interfaces[0], &interface_vtable, NULL, NULL, &error);
...
static void name_acquired_cb(GDBusConnection *connection, const char *name,
                             gpointer user_data) {
  OwnData *data = (OwnData *)user_data;
  GError *error = NULL;
  ...
  if (!terminal_app_handle_options(terminal_app_get(), data->options,
                                   TRUE /* do resume */, &error)) {
...
static void name_lost_cb(GDBusConnection *connection, const char *name,
                         gpointer user_data) {
  OwnData *data = (OwnData *)user_data;
  GError *error = NULL;
  char **envv;
  ...
  g_variant_builder_add(&builder, "@ay",
                        string_to_ay(data->options->default_working_dir));
  g_variant_builder_add(&builder, "@ay",
                        string_to_ay(data->options->display_name));
  g_variant_builder_add(&builder, "@ay",
                        string_to_ay(data->options->startup_id));

  string = g_string_new(NULL);
  envv = g_get_environ();
  for (i = 0; envv[i]; ++i) {
    if (i > 0) g_string_append_c(string, '\0');
  ...
  g_variant_builder_add(&builder, "@i",
                        g_variant_new_int32(data->options->initial_workspace));
  ...
  value = g_dbus_connection_call_sync(
      connection, data->factory_name, TERMINAL_FACTORY_SERVICE_PATH,
      TERMINAL_FACTORY_INTERFACE_NAME, "HandleArguments",
      g_variant_builder_end(&builder), G_VARIANT_TYPE("()"),
...
static char *get_factory_name_for_display(const char *display_name) {
  ...
  name = g_string_sized_new(strlen(TERMINAL_FACTORY_SERVICE_NAME_PREFIX) +
                            strlen(display_name) + 1 /* NUL */);
...
static int get_initial_workspace(void) {
  ...
  if (gdk_property_get(window, atom, cardinal_atom, 0, 8, FALSE, NULL, NULL,
                       NULL, &data)) {
    ret = *(int *)data;
    g_free(data);
  }
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < argc; ++i) argv_copy[i] = argv[i];
  argv_copy[i] = NULL;
  argc_copy = argc;
  ...
  if (home_dir == NULL || chdir(home_dir) < 0)
    if (chdir("/") < 0) g_warning("Could not change working directory.");

  options = terminal_options_parse(working_directory, NULL, startup_id, NULL,
                                   FALSE, FALSE, &argc, &argv, &error,
#ifdef HAVE_SMCLIENT
  ...
  if (options->use_factory) {
    ...
    owner_id = g_bus_own_name(G_BUS_TYPE_SESSION, data->factory_name,
                              G_BUS_NAME_OWNER_FLAGS_NONE, bus_acquired_cb,
                              name_acquired_cb, name_lost_cb, data, NULL);
