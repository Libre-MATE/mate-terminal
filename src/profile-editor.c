...
static const TerminalColorScheme color_schemes[] = {
    {N_("Black on light yellow"), {0, 0, 0, 1}, {1, 1, 0.866667, 1}},
    {N_("Black on white"), {0, 0, 0, 1}, {1, 1, 1, 1}},
    {N_("Gray on black"), {0.666667, 0.666667, 0.666667, 1}, {0, 0, 0, 1}},
    {N_("Green on black"), {0, 1, 0, 1}, {0, 0, 0, 1}},
    {N_("White on black"), {1, 1, 1, 1}, {0, 0, 0, 1}},
...
static void profile_forgotten_cb(TerminalProfile *profile, GtkWidget *editor);

static void profile_notify_sensitivity_cb(TerminalProfile *profile,
                                          GParamSpec *pspec, GtkWidget *editor);
...
static void widget_and_labels_set_sensitive(GtkWidget *widget,
                                            gboolean sensitive) {
  GList *labels, *i;
...
#define SET_SENSITIVE(name, setting)                                       \
  widget_and_labels_set_sensitive(profile_editor_get_widget(editor, name), \
                                  setting)

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_USE_CUSTOM_COMMAND) ||
      prop_name == I_(TERMINAL_PROFILE_CUSTOM_COMMAND)) {
    gboolean use_custom_command_locked = terminal_profile_property_locked(
    ...
    SET_SENSITIVE("custom-command-box",
                  terminal_profile_get_property_boolean(
                      profile, TERMINAL_PROFILE_USE_CUSTOM_COMMAND) &&
                      !terminal_profile_property_locked(
                          profile, TERMINAL_PROFILE_CUSTOM_COMMAND));
  }

  gtk_widget_hide(profile_editor_get_widget(
      editor, "darken-background-transparent-or-image-scale-label"));
  gtk_widget_show(profile_editor_get_widget(
      editor, "darken-background-transparent-scale-label"));
  gtk_widget_hide(
      profile_editor_get_widget(editor, "scroll-background-checkbutton"));
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_BACKGROUND_TYPE)) {
    gboolean bg_type_locked = terminal_profile_property_locked(
    ...
    if (bg_type == TERMINAL_BACKGROUND_TRANSPARENT) {
      SET_SENSITIVE("darken-background-vbox",
                    !terminal_profile_property_locked(
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_USE_SYSTEM_FONT) ||
    ...
    SET_SENSITIVE("font-hbox", !terminal_profile_get_property_boolean(
                                   profile, TERMINAL_PROFILE_USE_SYSTEM_FONT) &&
                                   !terminal_profile_property_locked(
                                       profile, TERMINAL_PROFILE_FONT));
    SET_SENSITIVE("system-font-checkbutton",
                  !terminal_profile_property_locked(
                      profile, TERMINAL_PROFILE_USE_SYSTEM_FONT));
  }

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_FOREGROUND_COLOR) ||
    ...
    bg_locked = terminal_profile_property_locked(
        profile, TERMINAL_PROFILE_BACKGROUND_COLOR);

    SET_SENSITIVE("foreground-colorpicker", !use_theme_colors && !fg_locked);
    SET_SENSITIVE("foreground-colorpicker-label",
                  !use_theme_colors && !fg_locked);
    SET_SENSITIVE("background-colorpicker", !use_theme_colors && !bg_locked);
    SET_SENSITIVE("background-colorpicker-label",
                  !use_theme_colors && !bg_locked);
    SET_SENSITIVE("color-scheme-combobox",
                  !use_theme_colors && !fg_locked && !bg_locked);
    SET_SENSITIVE("color-scheme-combobox-label",
                  !use_theme_colors && !fg_locked && !bg_locked);
  }

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_BOLD_COLOR) ||
      prop_name == I_(TERMINAL_PROFILE_BOLD_COLOR_SAME_AS_FG) ||
      prop_name == I_(TERMINAL_PROFILE_USE_THEME_COLORS)) {
    gboolean bold_locked, bold_same_as_fg_locked, bold_same_as_fg,
        use_theme_colors;

    use_theme_colors = terminal_profile_get_property_boolean(
        profile, TERMINAL_PROFILE_USE_THEME_COLORS);
    bold_locked =
        terminal_profile_property_locked(profile, TERMINAL_PROFILE_BOLD_COLOR);
    bold_same_as_fg_locked = terminal_profile_property_locked(
    ...
    SET_SENSITIVE("bold-color-same-as-fg-checkbox",
                  !use_theme_colors && !bold_same_as_fg_locked);
    SET_SENSITIVE("bold-colorpicker",
                  !use_theme_colors && !bold_locked && !bold_same_as_fg);
    SET_SENSITIVE("bold-colorpicker-label",
                  !use_theme_colors && ((!bold_same_as_fg && !bold_locked) ||
                                        !bold_same_as_fg_locked));
  }
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_DEFAULT_SHOW_MENUBAR))
    SET_SENSITIVE("show-menubar-checkbutton",
                  !terminal_profile_property_locked(
                      profile, TERMINAL_PROFILE_DEFAULT_SHOW_MENUBAR));

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_TITLE))
    SET_SENSITIVE("title-entry", !terminal_profile_property_locked(
                                     profile, TERMINAL_PROFILE_TITLE));

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_TITLE_MODE))
    SET_SENSITIVE("title-mode-combobox",
                  !terminal_profile_property_locked(
                      profile, TERMINAL_PROFILE_TITLE_MODE));
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_USE_CUSTOM_DEFAULT_SIZE) ||
      prop_name == I_(TERMINAL_PROFILE_DEFAULT_SIZE_COLUMNS) ||
      prop_name == I_(TERMINAL_PROFILE_DEFAULT_SIZE_ROWS)) {
    gboolean use_custom_default_size_locked = terminal_profile_property_locked(
        profile, TERMINAL_PROFILE_USE_CUSTOM_DEFAULT_SIZE);
    gboolean use_custom_default_size = terminal_profile_get_property_boolean(
        profile, TERMINAL_PROFILE_USE_CUSTOM_DEFAULT_SIZE);
    gboolean columns_locked = terminal_profile_property_locked(
        profile, TERMINAL_PROFILE_DEFAULT_SIZE_COLUMNS);
    gboolean rows_locked = terminal_profile_property_locked(
        profile, TERMINAL_PROFILE_DEFAULT_SIZE_ROWS);

    SET_SENSITIVE("use-custom-default-size-checkbutton",
                  !use_custom_default_size_locked);
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_SCROLLBAR_POSITION))
    SET_SENSITIVE("scrollbar-position-combobox",
                  !terminal_profile_property_locked(
                      profile, TERMINAL_PROFILE_SCROLLBAR_POSITION));

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_SCROLLBACK_LINES) ||
      prop_name == I_(TERMINAL_PROFILE_SCROLLBACK_UNLIMITED)) {
    gboolean scrollback_lines_locked = terminal_profile_property_locked(
        profile, TERMINAL_PROFILE_SCROLLBACK_LINES);
    gboolean scrollback_unlimited_locked = terminal_profile_property_locked(
        profile, TERMINAL_PROFILE_SCROLLBACK_UNLIMITED);
    gboolean scrollback_unlimited = terminal_profile_get_property_boolean(
        profile, TERMINAL_PROFILE_SCROLLBACK_UNLIMITED);

    SET_SENSITIVE("scrollback-label", !scrollback_lines_locked);
    SET_SENSITIVE("scrollback-box",
                  !scrollback_lines_locked && !scrollback_unlimited);
    SET_SENSITIVE("scrollback-unlimited-checkbutton",
                  !scrollback_lines_locked && !scrollback_unlimited_locked);
  }
  ...
  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_EXIT_ACTION))
    SET_SENSITIVE("exit-action-combobox",
                  !terminal_profile_property_locked(
                      profile, TERMINAL_PROFILE_EXIT_ACTION));

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_LOGIN_SHELL))
    SET_SENSITIVE("login-shell-checkbutton",
                  !terminal_profile_property_locked(
                      profile, TERMINAL_PROFILE_LOGIN_SHELL));

  if (!prop_name || prop_name == I_(TERMINAL_PROFILE_PALETTE)) {
    gboolean palette_locked =
        terminal_profile_property_locked(profile, TERMINAL_PROFILE_PALETTE);
...
static void color_scheme_combo_changed_cb(GtkWidget *combo, GParamSpec *pspec,
  ...
  if (i < G_N_ELEMENTS(color_schemes)) {
    g_signal_handlers_block_by_func(
        profile, G_CALLBACK(profile_colors_notify_scheme_combo_cb), combo);
    g_object_set(profile, TERMINAL_PROFILE_FOREGROUND_COLOR,
                 &color_schemes[i].foreground,
                 TERMINAL_PROFILE_BACKGROUND_COLOR,
                 &color_schemes[i].background, NULL);
...
static void profile_colors_notify_scheme_combo_cb(TerminalProfile *profile,
  ...
  fg = terminal_profile_get_property_boxed(profile,
                                           TERMINAL_PROFILE_FOREGROUND_COLOR);
  bg = terminal_profile_get_property_boxed(profile,
                                           TERMINAL_PROFILE_BACKGROUND_COLOR);
  ...
  /* If we didn't find a match, then we get the last combo box item which is
   * "custom" */

  g_signal_handlers_block_by_func(
      combo, G_CALLBACK(color_scheme_combo_changed_cb), profile);
  gtk_combo_box_set_active(GTK_COMBO_BOX(combo), i);
  g_signal_handlers_unblock_by_func(
      combo, G_CALLBACK(color_scheme_combo_changed_cb), profile);
}

static void palette_scheme_combo_changed_cb(GtkComboBox *combo,
                                            GParamSpec *pspec,
                                            TerminalProfile *profile) {
  int i;
...
static void profile_palette_notify_scheme_combo_cb(TerminalProfile *profile,
  ...
  g_signal_handlers_block_by_func(
      combo, G_CALLBACK(palette_scheme_combo_changed_cb), profile);
  gtk_combo_box_set_active(combo, i);
  g_signal_handlers_unblock_by_func(
      combo, G_CALLBACK(palette_scheme_combo_changed_cb), profile);
}

static void palette_color_notify_cb(GtkColorChooser *button, GParamSpec *pspec,
                                    TerminalProfile *profile) {
  GtkWidget *editor;
  GdkRGBA color;
  guint i;
  ...
  i = GPOINTER_TO_UINT(
      g_object_get_data(G_OBJECT(button), "palette-entry-index"));

  editor = gtk_widget_get_toplevel(GTK_WIDGET(button));
  g_signal_handlers_block_by_func(
      profile, G_CALLBACK(profile_palette_notify_colorpickers_cb), editor);
...
static void profile_palette_notify_colorpickers_cb(TerminalProfile *profile,
  ...
  for (i = 0; i < n_colors; i++) {
    ...
    g_signal_handlers_block_by_func(w, G_CALLBACK(palette_color_notify_cb),
                                    profile);
    gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(w), &colors[i]);
    g_signal_handlers_unblock_by_func(w, G_CALLBACK(palette_color_notify_cb),
                                      profile);
  }
}

static void custom_command_entry_changed_cb(GtkEntry *entry) {
  const char *command;
  GError *error = NULL;
...
static void visible_name_entry_changed_cb(GtkEntry *entry, GtkWindow *window) {
...
static void reset_compat_defaults_cb(GtkWidget *button,
                                     TerminalProfile *profile) {
...
static void init_color_scheme_menu(GtkWidget *widget) {
  ...
  for (i = 0; i < G_N_ELEMENTS(color_schemes); ++i)
    gtk_list_store_insert_with_values(store, &iter, -1, 0,
                                      _(color_schemes[i].name), -1);
  gtk_list_store_insert_with_values(store, &iter, -1, 0, _("Custom"), -1);
...
static char *format_percent_value(GtkScale *scale, double val, void *data) {
  return g_strdup_printf("%d%%", (int)(val * 100.0 + 0.5));
}

static void init_background_darkness_scale(GtkWidget *scale) {
  g_signal_connect(scale, "format-value", G_CALLBACK(format_percent_value),
                   NULL);
}

static void editor_response_cb(GtkWidget *editor, int response,
                               gpointer use_data) {
...
static void setup_background_filechooser(GtkWidget *filechooser,
                                         TerminalProfile *profile) {
  ...
  /* Start filechooser in $HOME instead of the current dir of the factory which
   * is "/" */
...
static void profile_editor_destroyed(GtkWidget *editor,
                                     TerminalProfile *profile) {
  g_signal_handlers_disconnect_by_func(
      profile, G_CALLBACK(profile_forgotten_cb), editor);
  g_signal_handlers_disconnect_by_func(
      profile, G_CALLBACK(profile_notify_sensitivity_cb), editor);
  g_signal_handlers_disconnect_matched(
      profile, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
...
static void terminal_profile_editor_focus_widget(GtkWidget *editor,
                                                 const char *widget_name) {
  GtkBuilder *builder;
  GtkWidget *widget, *page, *page_parent;

  if (widget_name == NULL) return;
  ...
  page = widget;
  while (page != NULL && (page_parent = gtk_widget_get_parent(page)) != NULL &&
  ...
  if (page != NULL && GTK_IS_NOTEBOOK(page_parent)) {
    ...
    gtk_notebook_set_current_page(notebook,
                                  gtk_notebook_page_num(notebook, page));
  }

  if (gtk_widget_is_sensitive(widget)) gtk_widget_grab_focus(widget);
}

static gboolean on_profile_editor_notebook_scroll_event(GtkWidget *widget,
...
void terminal_profile_edit(TerminalProfile *profile,
  ...
  gtk_builder_add_from_resource(builder,
                                ...
                                "ui/profile-preferences.ui",
                                &error);
  g_assert_no_error(error);

  editor =
      (GtkWidget *)gtk_builder_get_object(builder, "profile-editor-dialog");
  g_object_set_data_full(G_OBJECT(editor), "builder", builder,
  ...
  g_signal_connect(editor, "destroy", G_CALLBACK(profile_editor_destroyed),
                   profile);

  g_signal_connect(editor, "response", G_CALLBACK(editor_response_cb), NULL);
  ...
  w = (GtkWidget *)gtk_builder_get_object(builder,
                                          "background-image-filechooser");
  ...
  g_signal_connect(profile, "notify::" TERMINAL_PROFILE_FOREGROUND_COLOR,
                   G_CALLBACK(profile_colors_notify_scheme_combo_cb), w);
  g_signal_connect(profile, "notify::" TERMINAL_PROFILE_BACKGROUND_COLOR,
                   G_CALLBACK(profile_colors_notify_scheme_combo_cb), w);

#define CONNECT_WITH_FLAGS(name, prop, flags)   \
  terminal_util_bind_object_property_to_widget( \
      G_OBJECT(profile), prop,                  \
      (GtkWidget *)gtk_builder_get_object(builder, name), flags)
#define CONNECT(name, prop) CONNECT_WITH_FLAGS(name, prop, 0)
#define SET_ENUM_VALUE(name, value)                                      \
  g_object_set_data(gtk_builder_get_object(builder, name), "enum-value", \
                    GINT_TO_POINTER(value))

  w = GTK_WIDGET(gtk_builder_get_object(builder, "custom-command-entry"));
  custom_command_entry_changed_cb(GTK_ENTRY(w));
  g_signal_connect(w, "changed", G_CALLBACK(custom_command_entry_changed_cb),
                   NULL);
  w = GTK_WIDGET(gtk_builder_get_object(builder, "profile-name-entry"));
  g_signal_connect(w, "changed", G_CALLBACK(visible_name_entry_changed_cb),
                   editor);

  g_signal_connect(
      gtk_builder_get_object(builder, "reset-compat-defaults-button"),
      "clicked", G_CALLBACK(reset_compat_defaults_cb), profile);
  ...
  CONNECT("background-image-filechooser",
          TERMINAL_PROFILE_BACKGROUND_IMAGE_FILE);
  CONNECT("backspace-binding-combobox", TERMINAL_PROFILE_BACKSPACE_BINDING);
  CONNECT("bold-color-same-as-fg-checkbox",
  ...
  CONNECT("default-size-columns-spinbutton",
          TERMINAL_PROFILE_DEFAULT_SIZE_COLUMNS);
  ...
  CONNECT("scrollback-unlimited-checkbutton",
          TERMINAL_PROFILE_SCROLLBACK_UNLIMITED);
  ...
  CONNECT("scroll-on-keystroke-checkbutton",
  ...
  CONNECT("use-custom-command-checkbutton",
          TERMINAL_PROFILE_USE_CUSTOM_COMMAND);
  CONNECT("use-custom-default-size-checkbutton",
          TERMINAL_PROFILE_USE_CUSTOM_DEFAULT_SIZE);
  ...
  CONNECT_WITH_FLAGS("bell-checkbutton", TERMINAL_PROFILE_SILENT_BELL,
                     FLAG_INVERT_BOOL);
  /* CONNECT_WITH_FLAGS ("copy-checkbutton", TERMINAL_PROFILE_COPY_SELECTION,
   * FLAG_INVERT_BOOL); */
...
#undef SET_ENUM_VALUE
  ...
  g_signal_connect(profile, "notify", G_CALLBACK(profile_notify_sensitivity_cb),
                   editor);
  g_signal_connect(profile, "forgotten", G_CALLBACK(profile_forgotten_cb),
                   editor);
  ...
  g_signal_connect(w, "scroll-event",
                   G_CALLBACK(on_profile_editor_notebook_scroll_event), NULL);
