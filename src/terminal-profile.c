...
static const GdkRGBA
    terminal_palettes[TERMINAL_PALETTE_N_BUILTINS][TERMINAL_PALETTE_SIZE] = {
        /* Tango palette */
        {
            {0, 0, 0, 1},
            {0.8, 0, 0, 1},
...
enum { FORGOTTEN, LAST_SIGNAL };

static void terminal_profile_finalize(GObject *object);
static void terminal_profile_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void ensure_pixbuf_property(TerminalProfile *profile, guint path_prop_id,
                                   guint pixbuf_prop_id, gboolean *load_failed);
...
static gboolean rgba_equal(const GdkRGBA *a, const GdkRGBA *b) {
  gdouble dr, dg, db, da;
...
static gboolean palette_cmp(const GdkRGBA *ca, const GdkRGBA *cb) {
  guint i;

  for (i = 0; i < TERMINAL_PALETTE_SIZE; ++i)
    if (!rgba_equal(&ca[i], &cb[i])) return FALSE;

  return TRUE;
}

static GParamSpec *get_pspec_from_name(TerminalProfile *profile,
  ...
  if (pspec && pspec->owner_type != TERMINAL_TYPE_PROFILE) pspec = NULL;

  return pspec;
}

static const GValue *get_prop_value_from_prop_name(TerminalProfile *profile,
                                                   const char *prop_name) {
  ...
  if (G_UNLIKELY(pspec->param_id == PROP_BACKGROUND_IMAGE))
    ensure_pixbuf_property(profile, PROP_BACKGROUND_IMAGE_FILE,
                           PROP_BACKGROUND_IMAGE,
...
static void set_value_from_palette(GValue *ret_value, const GdkRGBA *colors,
                                   guint n_colors) {
  GValueArray *array;
  ...
  for (i = 0; i < max_n_colors; ++i) g_value_array_append(array, NULL);
...
static int values_equal(GParamSpec *pspec, const GValue *va, const GValue *vb) {
  ...
  if (g_param_values_cmp(pspec, va, vb) == 0) return TRUE;
  ...
  if (G_PARAM_SPEC_VALUE_TYPE(pspec) == PANGO_TYPE_FONT_DESCRIPTION)
    return pango_font_description_equal(g_value_get_boxed(va),
                                        g_value_get_boxed(vb));

  if (G_IS_PARAM_SPEC_VALUE_ARRAY(pspec) &&
      G_PARAM_SPEC_VALUE_TYPE(G_PARAM_SPEC_VALUE_ARRAY(pspec)->element_spec) ==
          GDK_TYPE_RGBA) {
    GValueArray *ara, *arb;
    guint i;
    ...
    if (!ara || !arb || ara->n_values != arb->n_values) return FALSE;
...
static void ensure_pixbuf_property(TerminalProfile *profile, guint path_prop_id,
  ...
  if (!path_utf8 || !path_utf8[0]) goto failed;
...
static void terminal_profile_reset_property_internal(TerminalProfile *profile,
                                                     GParamSpec *pspec,
                                                     gboolean notify) {
  TerminalProfilePrivate *priv = profile->priv;
  GValue value_ = {
  ...
  /* A few properties don't have defaults via the param spec; set them
   * explicitly */
  switch (pspec->param_id) {
    ...
    case PROP_BOLD_COLOR: {
      ...
      if (!gdk_rgba_parse(&color, DEFAULT_FOREGROUND_COLOR)) return;
      color.alpha = 1.0;
    ...
    case PROP_BACKGROUND_COLOR: {
      ...
      if (!gdk_rgba_parse(&color, DEFAULT_BACKGROUND_COLOR)) return;
      color.alpha = 1.0;
      g_value_set_boxed(value, &color);
      break;
    }
...
static void terminal_profile_gsettings_notify_cb(GSettings *settings,
                                                 gchar *key,
                                                 gpointer user_data) {
  ...
  _terminal_debug_print(
      TERMINAL_DEBUG_PROFILE, "GSettings notification for key %s [%s]\n", key,
  ...
  if (!pspec) return; /* ignore unknown keys, for future extensibility */

  priv->locked[pspec->param_id] = !g_settings_is_writable(settings, key);

  settings_value = g_settings_get_value(settings, key);
  if (!settings_value) return;

  g_value_init(&value, G_PARAM_SPEC_VALUE_TYPE(pspec));

  if (G_IS_PARAM_SPEC_BOOLEAN(pspec)) {
    if (!g_variant_is_of_type(settings_value, G_VARIANT_TYPE_BOOLEAN)) goto out;

    g_value_set_boolean(&value, g_variant_get_boolean(settings_value));
  } else if (G_IS_PARAM_SPEC_STRING(pspec)) {
    if (!g_variant_is_of_type(settings_value, G_VARIANT_TYPE_STRING)) goto out;
    ...
    g_value_take_boxed(&value, pango_font_description_from_string(
                                   g_variant_get_string(settings_value, NULL)));
  } else if (G_IS_PARAM_SPEC_DOUBLE(pspec)) {
    if (!g_variant_is_of_type(settings_value, G_VARIANT_TYPE_DOUBLE)) goto out;
    ...
    color_strings =
        g_strsplit(g_variant_get_string(settings_value, NULL), ":", -1);
    if (!color_strings) goto out;
  ...
  if (g_param_value_validate(pspec, &value)) {
    _terminal_debug_print(TERMINAL_DEBUG_PROFILE,
                          "Invalid value in GSettings for key %s was changed "
                          "to comply with pspec %s\n",
                          key, pspec->name);
   ...
   */
  equal = values_equal(
      pspec, &value, g_value_array_get_nth(priv->properties, pspec->param_id));
  ...
  _TERMINAL_DEBUG_IF(TERMINAL_DEBUG_PROFILE) {
    if (!equal)
      _terminal_debug_print(TERMINAL_DEBUG_PROFILE,
                            ...
                            "  new: %s\n",
                            pspec->name,
                            g_strdup_value_contents(g_value_array_get_nth(
                                priv->properties, pspec->param_id)),
...
static void terminal_profile_gsettings_changeset_add(TerminalProfile *profile,
                                                     GSettings *changeset,
                                                     GParamSpec *pspec) {
  TerminalProfilePrivate *priv = profile->priv;
  char *key;
  ...
  if (G_IS_PARAM_SPEC_BOOLEAN(pspec))
    ...
    eval = g_enum_get_value(G_PARAM_SPEC_ENUM(pspec)->enum_class,
    ...
    if (!color) goto cleanup;

    g_snprintf(str, sizeof(str), "#%04X%04X%04X", (guint)(color->red * 65535),
    ...
    /* We need to do this ourselves, because the
     * gtk_color_selection_palette_to_string does not carry all the bytes, and
     * xterm's palette is messed up...
     */
    ...
    string = g_string_sized_new(n_colors * (1 /* # */ + 3 * 4) +
                                n_colors /* : separators and terminating \0 */);
    for (i = 0; i < n_colors; ++i) {
      GdkRGBA *color;
      ...
      g_string_append_printf(
          string, "#%04X%04X%04X", (guint)(color->red * 65535),
          (guint)(color->green * 65535), (guint)(color->blue * 65535));
    }
...
static void terminal_profile_save(TerminalProfile *profile) {
  TerminalProfilePrivate *priv = profile->priv;
  GSettings *changeset;
  GSList *l;
  gchar *concat;
  ...
  for (l = priv->dirty_pspecs; l != NULL; l = l->next) {
    ...
    if (pspec->owner_type != TERMINAL_TYPE_PROFILE) continue;

    if ((pspec->flags & G_PARAM_WRITABLE) == 0) continue;
...
static gboolean terminal_profile_save_idle_cb(TerminalProfile *profile) {
...
static void terminal_profile_schedule_save(TerminalProfile *profile,
                                           GParamSpec *pspec) {
  TerminalProfilePrivate *priv = profile->priv;
  ...
  if (priv->save_idle_id != 0) return;

  priv->save_idle_id =
      g_idle_add((GSourceFunc)terminal_profile_save_idle_cb, profile);
}
...
static void terminal_profile_init(TerminalProfile *profile) {
  ...
  for (i = 0; i < LAST_PROP; ++i) g_value_array_append(priv->properties, NULL);

  pspecs = g_object_class_list_properties(
      G_OBJECT_CLASS(TERMINAL_PROFILE_GET_CLASS(profile)), &n_pspecs);
  ...
  terminal_profile_reset_property_internal(
      profile,
      g_object_class_find_property(object_class,
                                   TERMINAL_PROFILE_FOREGROUND_COLOR),
      FALSE);
  terminal_profile_reset_property_internal(
      profile,
      g_object_class_find_property(object_class, TERMINAL_PROFILE_BOLD_COLOR),
      FALSE);
  terminal_profile_reset_property_internal(
      profile,
      g_object_class_find_property(object_class,
                                   TERMINAL_PROFILE_BACKGROUND_COLOR),
      FALSE);
  terminal_profile_reset_property_internal(
      profile,
      g_object_class_find_property(object_class, TERMINAL_PROFILE_FONT), FALSE);
  terminal_profile_reset_property_internal(
      profile,
      g_object_class_find_property(object_class, TERMINAL_PROFILE_PALETTE),
      FALSE);
}

static GObject *terminal_profile_constructor(
  ...
  object = G_OBJECT_CLASS(terminal_profile_parent_class)
  ...
  g_signal_connect(priv->settings, concat,
                   G_CALLBACK(terminal_profile_gsettings_notify_cb), profile);

  g_free(concat);

  /* Now load those properties from GSettings that were not set as construction
   * params */
  pspecs = g_object_class_list_properties(
      G_OBJECT_CLASS(TERMINAL_PROFILE_GET_CLASS(profile)), &n_pspecs);
  for (i = 0; i < n_pspecs; ++i) {
...
static void terminal_profile_finalize(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(
      priv->settings, G_CALLBACK(terminal_profile_gsettings_notify_cb),
      profile);
...
static void terminal_profile_get_property(GObject *object, guint prop_id,
  ...
  /* Note: When adding things here, do the same in
   * get_prop_value_from_prop_name! */
  switch (prop_id) {
...
static void terminal_profile_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_BACKGROUND_IMAGE_FILE:
      ...
      g_value_set_object(
          g_value_array_get_nth(priv->properties, PROP_BACKGROUND_IMAGE), NULL);
...
static void terminal_profile_notify(GObject *object, GParamSpec *pspec) {
  TerminalProfilePrivate *priv = TERMINAL_PROFILE(object)->priv;
  void (*notify)(GObject *, GParamSpec *) =
      G_OBJECT_CLASS(terminal_profile_parent_class)->notify;

  _terminal_debug_print(TERMINAL_DEBUG_PROFILE,
                        "Property notification for prop %s\n", pspec->name);

  if (notify) notify(object, pspec);
...
static void terminal_profile_class_init(TerminalProfileClass *klass) {
  ...
  signals[FORGOTTEN] = g_signal_new(
      "forgotten", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(TerminalProfileClass, forgotten), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  /* gsettings_key -> pspec hash */
  klass->gsettings_keys = g_hash_table_new(g_str_hash, g_str_equal);

#define TERMINAL_PROFILE_PSPEC_STATIC \
...
#define TERMINAL_PROFILE_PROPERTY(propId, propSpec, propGSettings)      \
  {                                                                     \
    ...
    g_param_spec_set_qdata(pspec, gsettings_key_quark,                  \
                           (gpointer)propGSettings);                    \
    g_hash_table_insert(klass->gsettings_keys, (gpointer)propGSettings, \
                        pspec);                                         \
  }
...
#define TERMINAL_PROFILE_PROPERTY_BOOLEAN(prop, propDefault, propGSettings)    \
  TERMINAL_PROFILE_PROPERTY(                                                   \
      PROP_##prop,                                                             \
      g_param_spec_boolean(TERMINAL_PROFILE_##prop, NULL, NULL, propDefault,   \
...
#define TERMINAL_PROFILE_PROPERTY_BOXED(prop, propType, propGSettings)       \
  TERMINAL_PROFILE_PROPERTY(                                                 \
      PROP_##prop,                                                           \
      g_param_spec_boxed(TERMINAL_PROFILE_##prop, NULL, NULL, propType,      \
                         G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC), \
      propGSettings)

#define TERMINAL_PROFILE_PROPERTY_DOUBLE(prop, propMin, propMax, propDefault, \
                                         propGSettings)                       \
  TERMINAL_PROFILE_PROPERTY(                                                  \
      PROP_##prop,                                                            \
      g_param_spec_double(TERMINAL_PROFILE_##prop, NULL, NULL, propMin,       \
                          propMax, propDefault,                               \
                          G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC), \
      propGSettings)

#define TERMINAL_PROFILE_PROPERTY_ENUM(prop, propType, propDefault,         \
                                       propGSettings)                       \
  TERMINAL_PROFILE_PROPERTY(                                                \
      PROP_##prop,                                                          \
      g_param_spec_enum(TERMINAL_PROFILE_##prop, NULL, NULL, propType,      \
                        propDefault,                                        \
                        G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC), \
      propGSettings)

#define TERMINAL_PROFILE_PROPERTY_INT(prop, propMin, propMax, propDefault,    \
                                      propGSettings)                          \
  TERMINAL_PROFILE_PROPERTY(                                                  \
      PROP_##prop,                                                            \
      g_param_spec_int(TERMINAL_PROFILE_##prop, NULL, NULL, propMin, propMax, \
                       propDefault,                                           \
                       G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC),    \
      propGSettings)
...
#define TERMINAL_PROFILE_PROPERTY_OBJECT(prop, propType)                 \
  TERMINAL_PROFILE_PROPERTY_OUTER(                                       \
      PROP_##prop,                                                       \
      g_param_spec_object(TERMINAL_PROFILE_##prop, NULL, NULL, propType, \
                          G_PARAM_READABLE | TERMINAL_PROFILE_PSPEC_STATIC))

#define TERMINAL_PROFILE_PROPERTY_STRING(prop, propDefault, propGSettings)    \
  TERMINAL_PROFILE_PROPERTY(                                                  \
      PROP_##prop,                                                            \
      g_param_spec_string(TERMINAL_PROFILE_##prop, NULL, NULL, propDefault,   \
                          G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC), \
      propGSettings)
  ...
  TERMINAL_PROFILE_PROPERTY_OUTER(                                          \
      ...
      g_param_spec_string(TERMINAL_PROFILE_##prop, NULL, NULL, propDefault, \
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |      \
                              TERMINAL_PROFILE_PSPEC_STATIC))

#define TERMINAL_PROFILE_PROPERTY_VALUE_ARRAY_BOXED(              \
    prop, propElementName, propElementType, propGSettings)        \
  TERMINAL_PROFILE_PROPERTY(                                      \
      PROP_##prop,                                                \
      g_param_spec_value_array(                                   \
          TERMINAL_PROFILE_##prop, NULL, NULL,                    \
          g_param_spec_boxed(                                     \
              propElementName, NULL, NULL, propElementType,       \
              G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC), \
          G_PARAM_READWRITE | TERMINAL_PROFILE_PSPEC_STATIC),     \
      propGSettings)

  TERMINAL_PROFILE_PROPERTY_BOOLEAN(ALLOW_BOLD, DEFAULT_ALLOW_BOLD,
                                    KEY_ALLOW_BOLD);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(BOLD_COLOR_SAME_AS_FG,
                                    DEFAULT_BOLD_COLOR_SAME_AS_FG,
                                    KEY_BOLD_COLOR_SAME_AS_FG);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(DEFAULT_SHOW_MENUBAR,
                                    DEFAULT_DEFAULT_SHOW_MENUBAR,
                                    KEY_DEFAULT_SHOW_MENUBAR);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(LOGIN_SHELL, DEFAULT_LOGIN_SHELL,
                                    KEY_LOGIN_SHELL);
  ...
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(SCROLLBACK_UNLIMITED,
                                    DEFAULT_SCROLLBACK_UNLIMITED,
                                    KEY_SCROLLBACK_UNLIMITED);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(SCROLL_ON_KEYSTROKE,
                                    DEFAULT_SCROLL_ON_KEYSTROKE,
                                    KEY_SCROLL_ON_KEYSTROKE);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(SCROLL_ON_OUTPUT, DEFAULT_SCROLL_ON_OUTPUT,
                                    KEY_SCROLL_ON_OUTPUT);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(SILENT_BELL, DEFAULT_SILENT_BELL,
                                    KEY_SILENT_BELL);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(COPY_SELECTION, DEFAULT_COPY_SELECTION,
                                    KEY_COPY_SELECTION);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(
      USE_CUSTOM_COMMAND, DEFAULT_USE_CUSTOM_COMMAND, KEY_USE_CUSTOM_COMMAND);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(USE_CUSTOM_DEFAULT_SIZE,
                                    DEFAULT_USE_CUSTOM_DEFAULT_SIZE,
                                    KEY_USE_CUSTOM_DEFAULT_SIZE);
  ...
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(USE_SYSTEM_FONT, DEFAULT_USE_SYSTEM_FONT,
                                    KEY_USE_SYSTEM_FONT);
  TERMINAL_PROFILE_PROPERTY_BOOLEAN(USE_THEME_COLORS, DEFAULT_USE_THEME_COLORS,
                                    KEY_USE_THEME_COLORS);

  TERMINAL_PROFILE_PROPERTY_BOXED(BACKGROUND_COLOR, GDK_TYPE_RGBA,
  ...
  TERMINAL_PROFILE_PROPERTY_BOXED(FOREGROUND_COLOR, GDK_TYPE_RGBA,
                                  KEY_FOREGROUND_COLOR);

  /* 0.0 = normal bg, 1.0 = all black bg, 0.5 = half darkened */
  TERMINAL_PROFILE_PROPERTY_DOUBLE(BACKGROUND_DARKNESS, 0.0, 1.0,
                                   DEFAULT_BACKGROUND_DARKNESS,
                                   KEY_BACKGROUND_DARKNESS);

  TERMINAL_PROFILE_PROPERTY_ENUM(BACKGROUND_TYPE, TERMINAL_TYPE_BACKGROUND_TYPE,
                                 DEFAULT_BACKGROUND_TYPE, KEY_BACKGROUND_TYPE);
  TERMINAL_PROFILE_PROPERTY_ENUM(BACKSPACE_BINDING, VTE_TYPE_ERASE_BINDING,
                                 DEFAULT_BACKSPACE_BINDING,
                                 KEY_BACKSPACE_BINDING);
  TERMINAL_PROFILE_PROPERTY_ENUM(CURSOR_BLINK_MODE, VTE_TYPE_CURSOR_BLINK_MODE,
                                 DEFAULT_CURSOR_BLINK_MODE,
                                 KEY_CURSOR_BLINK_MODE);
  TERMINAL_PROFILE_PROPERTY_ENUM(CURSOR_SHAPE, VTE_TYPE_CURSOR_SHAPE,
                                 DEFAULT_CURSOR_SHAPE, KEY_CURSOR_SHAPE);
  TERMINAL_PROFILE_PROPERTY_ENUM(DELETE_BINDING, VTE_TYPE_ERASE_BINDING,
                                 DEFAULT_DELETE_BINDING, KEY_DELETE_BINDING);
  ...
  TERMINAL_PROFILE_PROPERTY_ENUM(
      SCROLLBAR_POSITION, TERMINAL_TYPE_SCROLLBAR_POSITION,
      DEFAULT_SCROLLBAR_POSITION, KEY_SCROLLBAR_POSITION);
  TERMINAL_PROFILE_PROPERTY_ENUM(TITLE_MODE, TERMINAL_TYPE_TITLE_MODE,
                                 DEFAULT_TITLE_MODE, KEY_TITLE_MODE);

  TERMINAL_PROFILE_PROPERTY_INT(DEFAULT_SIZE_COLUMNS, 1, 1024,
                                DEFAULT_DEFAULT_SIZE_COLUMNS,
                                KEY_DEFAULT_SIZE_COLUMNS);
  TERMINAL_PROFILE_PROPERTY_INT(DEFAULT_SIZE_ROWS, 1, 1024,
                                DEFAULT_DEFAULT_SIZE_ROWS,
                                KEY_DEFAULT_SIZE_ROWS);
  TERMINAL_PROFILE_PROPERTY_INT(SCROLLBACK_LINES, 1, G_MAXINT,
                                DEFAULT_SCROLLBACK_LINES, KEY_SCROLLBACK_LINES);

  TERMINAL_PROFILE_PROPERTY_OBJECT(BACKGROUND_IMAGE, GDK_TYPE_PIXBUF);

  TERMINAL_PROFILE_PROPERTY_STRING_CO(NAME, DEFAULT_NAME);

  TERMINAL_PROFILE_PROPERTY_STRING(BACKGROUND_IMAGE_FILE,
                                   DEFAULT_BACKGROUND_IMAGE_FILE,
                                   KEY_BACKGROUND_IMAGE_FILE);
  TERMINAL_PROFILE_PROPERTY_STRING(CUSTOM_COMMAND, DEFAULT_CUSTOM_COMMAND,
                                   KEY_CUSTOM_COMMAND);
  TERMINAL_PROFILE_PROPERTY_STRING(TITLE, _(DEFAULT_TITLE), KEY_TITLE);
  TERMINAL_PROFILE_PROPERTY_STRING(VISIBLE_NAME, _(DEFAULT_VISIBLE_NAME),
                                   KEY_VISIBLE_NAME);
  TERMINAL_PROFILE_PROPERTY_STRING(WORD_CHARS, DEFAULT_WORD_CHARS,
                                   KEY_WORD_CHARS);

  TERMINAL_PROFILE_PROPERTY_VALUE_ARRAY_BOXED(PALETTE, "palette-color",
                                              GDK_TYPE_RGBA, KEY_PALETTE);
}

/* Semi-Public API */

TerminalProfile *_terminal_profile_new(const char *name) {
  return g_object_new(TERMINAL_TYPE_PROFILE, "name", name, NULL);
}
...
TerminalProfile *_terminal_profile_clone(TerminalProfile *base_profile,
  ...
  pspecs = g_object_class_list_properties(
      G_OBJECT_CLASS(TERMINAL_PROFILE_GET_CLASS(base_profile)), &n_pspecs);
  ...
  for (i = 0; i < n_params; ++i) g_value_unset(&params[i].value);
...
gboolean terminal_profile_get_property_boolean(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_BOOLEAN(value)) return FALSE;
...
gconstpointer terminal_profile_get_property_boxed(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_BOXED(value)) return NULL;
...
double terminal_profile_get_property_double(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_DOUBLE(value)) return 0.0;
...
int terminal_profile_get_property_enum(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_ENUM(value)) return 0;
...
int terminal_profile_get_property_int(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_INT(value)) return 0;
...
gpointer terminal_profile_get_property_object(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_OBJECT(value)) return NULL;
...
const char *terminal_profile_get_property_string(TerminalProfile *profile,
  ...
  if (!value || !G_VALUE_HOLDS_STRING(value)) return NULL;
...
void terminal_profile_reset_property(TerminalProfile *profile,
  ...
  if (!pspec || (pspec->flags & G_PARAM_WRITABLE) == 0) return;

  terminal_profile_reset_property_internal(profile, pspec, TRUE);
}

gboolean terminal_profile_get_palette(TerminalProfile *profile, GdkRGBA *colors,
                                      guint *n_colors) {
  TerminalProfilePrivate *priv;
  GValueArray *array;
  guint i, n;
  ...
  priv = profile->priv;
  array =
      g_value_get_boxed(g_value_array_get_nth(priv->properties, PROP_PALETTE));
  if (!array) return FALSE;

  n = MIN(array->n_values, *n_colors);
  for (i = 0; i < n; ++i) {
    GdkRGBA *color = g_value_get_boxed(g_value_array_get_nth(array, i));
    if (!color) continue; /* shouldn't happen!! */
...
void terminal_profile_set_palette_builtin(TerminalProfile *profile, guint n) {
  GValue value = {
      0,
  };
...
gboolean terminal_profile_modify_palette_entry(TerminalProfile *profile,
  ...
  array =
      g_value_get_boxed(g_value_array_get_nth(priv->properties, PROP_PALETTE));
  if (!array || i >= array->n_values) return FALSE;

  value = g_value_array_get_nth(array, i);
  old_color = g_value_get_boxed(value);
  if (!old_color || !rgba_equal(old_color, color)) {
    g_value_set_boxed(value, color);
