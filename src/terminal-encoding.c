...
gboolean terminal_encoding_is_valid(TerminalEncoding *encoding) {
  ...
  if (encoding->validity_checked) return encoding->valid;
  ...
  _TERMINAL_DEBUG_IF(TERMINAL_DEBUG_ENCODINGS) {
    if (!encoding->valid) {
      ...
      _terminal_debug_print(TERMINAL_DEBUG_ENCODINGS, " input  \"%s\"\n",
                            ascii_sample);
      _terminal_debug_print(TERMINAL_DEBUG_ENCODINGS,
                            " output \"%s\" bytes read %" G_GSIZE_FORMAT
                            " written %" G_GSIZE_FORMAT "\n",
                            converted ? converted : "(null)", bytes_read,
                            bytes_written);
      if (error)
        _terminal_debug_print(TERMINAL_DEBUG_ENCODINGS, " Error: %s\n",
                              error->message);
    } else
      _terminal_debug_print(TERMINAL_DEBUG_ENCODINGS,
                            "Encoding %s is valid\n\n",
...
GType terminal_encoding_get_type(void) {
  ...
  if (G_UNLIKELY(type == 0)) {
    type = g_boxed_type_register_static(
        I_("TerminalEncoding"), (GBoxedCopyFunc)terminal_encoding_ref,
...
static void update_active_encodings_gsettings(void) {
  ...
  g_settings_set_strv(settings_global, "active-encodings",
                      (const gchar **)strings->data);
...
static void response_callback(GtkWidget *window, int id,
...
enum { COLUMN_NAME, COLUMN_CHARSET, COLUMN_DATA, N_COLUMNS };
...
static void button_clicked_cb(GtkWidget *button, EncodingDialogData *data) {
  ...
  if (!gtk_tree_selection_get_selected(selection, &model, &filter_iter)) return;
...
static void liststore_insert_encoding(gpointer key, TerminalEncoding *encoding,
                                      GtkListStore *store) {
  GtkTreeIter iter;

  if (!terminal_encoding_is_valid(encoding)) return;

  gtk_list_store_insert_with_values(
      store, &iter, -1, COLUMN_CHARSET, terminal_encoding_get_charset(encoding),
      COLUMN_NAME, encoding->name, COLUMN_DATA, encoding, -1);
}

static gboolean filter_active_encodings(GtkTreeModel *child_model,
...
static GtkTreeModel *encodings_create_treemodel(GtkListStore *base_store,
                                                gboolean active) {
  GtkTreeModel *model;
...
static void encodings_list_changed_cb(TerminalApp *app,
                                      EncodingDialogData *data) {
  gtk_list_store_clear(data->base_store);

  g_hash_table_foreach(terminal_app_get_encodings(app),
                       (GHFunc)liststore_insert_encoding, data->base_store);
}

static void encoding_dialog_data_free(EncodingDialogData *data) {
  g_signal_handlers_disconnect_by_func(
      terminal_app_get(), G_CALLBACK(encodings_list_changed_cb), data);
...
void terminal_encoding_dialog_show(GtkWindow *transient_parent) {
  ...
  if (!terminal_util_load_builder_resource(
          ...
          "ui/encodings-dialog.ui",
          "encodings-dialog", &data->dialog, "add-button", &data->add_button,
          "remove-button", &data->remove_button, "available-treeview",
          &data->available_tree_view, "displayed-treeview",
          &data->active_tree_view, NULL)) {
    g_free(data);
    return;
  }

  g_object_set_data_full(G_OBJECT(data->dialog), "GT::Data", data,
                         (GDestroyNotify)encoding_dialog_data_free);
  ...
  g_signal_connect(data->dialog, "response", G_CALLBACK(response_callback),
                   data);

  /* buttons */
  g_signal_connect(data->add_button, "clicked", G_CALLBACK(button_clicked_cb),
                   data);
  ...
  data->available_selection =
      gtk_tree_view_get_selection(data->available_tree_view);
  ...
  data->base_store = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING,
  ...
  g_signal_connect(data->dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &encoding_dialog);
}
...
GHashTable *terminal_encodings_get_builtins(void) {
  ...
  encoding = terminal_encoding_new("current", _("Current Locale"), FALSE, TRUE);
  g_hash_table_insert(encodings_hashtable,
                      (gpointer)terminal_encoding_get_id(encoding), encoding);

  for (i = 0; i < G_N_ELEMENTS(encodings); ++i) {
    encoding = terminal_encoding_new(encodings[i].charset, _(encodings[i].name),
                                     FALSE, FALSE);
    g_hash_table_insert(encodings_hashtable,
                        (gpointer)terminal_encoding_get_id(encoding), encoding);
  }
