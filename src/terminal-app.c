...
enum { QUIT, PROFILE_LIST_CHANGED, ENCODING_LIST_CHANGED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

enum { COL_PROFILE, NUM_COLUMNS };

enum { SOURCE_DEFAULT = 0, SOURCE_SESSION = 1 };
 ...
 */
static gboolean gsettings_append_strv(GSettings *settings, const gchar *key,
                                      const gchar *value) {
  gchar **old;
  gchar **new;
  gint size;
...
static gboolean gsettings_remove_all_from_strv(GSettings *settings,
                                               const gchar *key,
                                               const gchar *value) {
  GArray *array;
  gchar **old;
...
static GdkScreen *terminal_app_get_screen_by_display_name(
  ...
  if (display_name == NULL)
    ...
    for (l = displays; l != NULL; l = l->next) {
      ...
      if ((period && strncmp(gdk_display_get_name(disp), display_name,
                             period - display_name) == 0) ||
          (period == NULL &&
           strcmp(gdk_display_get_name(disp), display_name) == 0)) {
        display = disp;
        break;
      }
    }
    ...
    if (display == NULL)
      display =
          gdk_display_open(display_name); /* FIXME we never close displays */
  }
...
static int terminal_app_get_workspace_for_window(TerminalWindow *window) {
  int ret = -1;
  guchar *data = NULL;
  GdkAtom atom;
  ...
  gdk_property_get(gtk_widget_get_window(GTK_WIDGET(window)), atom,
                   cardinal_atom, 0, 8, FALSE, NULL, NULL, NULL, &data);

  if (data) ret = *(int *)data;
...
static int profiles_alphabetic_cmp(gconstpointer pa, gconstpointer pb) {
  TerminalProfile *a = (TerminalProfile *)pa;
  TerminalProfile *b = (TerminalProfile *)pb;
  int result;

  result = g_utf8_collate(
      terminal_profile_get_property_string(a, TERMINAL_PROFILE_VISIBLE_NAME),
      terminal_profile_get_property_string(b, TERMINAL_PROFILE_VISIBLE_NAME));
  if (result == 0)
    result =
        strcmp(terminal_profile_get_property_string(a, TERMINAL_PROFILE_NAME),
...
static void profiles_lookup_by_visible_name_foreach(gpointer key,
                                                    gpointer value,
                                                    gpointer data) {
  LookupInfo *info = data;
  const char *name;

  name = terminal_profile_get_property_string(value,
                                              TERMINAL_PROFILE_VISIBLE_NAME);
  if (name && strcmp(info->target, name) == 0) info->result = value;
}

static void terminal_window_destroyed(TerminalWindow *window,
                                      TerminalApp *app) {
  app->windows = g_list_remove(app->windows, window);

  if (app->windows == NULL) g_signal_emit(app, signals[QUIT], 0);
}

static TerminalProfile *terminal_app_create_profile(TerminalApp *app,
                                                    const char *name) {
  ...
  g_hash_table_insert(app->profiles,
                      g_strdup(terminal_profile_get_property_string(
                          profile, TERMINAL_PROFILE_NAME)),
                      profile /* adopts the refcount */);

  if (app->default_profile == NULL && app->default_profile_id != NULL &&
      strcmp(app->default_profile_id, terminal_profile_get_property_string(
                                          profile, TERMINAL_PROFILE_NAME)) ==
          0) {
...
static void terminal_app_delete_profile(TerminalProfile *profile) {
  ...
  gsettings_remove_all_from_strv(settings_global, PROFILE_LIST_KEY,
                                 profile_name);
...
static void terminal_app_profile_cell_data_func(GtkTreeViewColumn *tree_column,
...
static int terminal_app_profile_sort_func(GtkTreeModel *model, GtkTreeIter *a,
...
static /* ref */ GtkTreeModel *terminal_app_get_profile_liststore(
    TerminalApp *app, TerminalProfile *selected_profile,
    GtkTreeIter *selected_profile_iter, gboolean *selected_profile_iter_set) {
  GtkListStore *store;
  GtkTreeIter iter;
  GList *profiles, *l;
  ...
  *selected_profile_iter_set = FALSE;
  ...
  if (selected_profile && _terminal_profile_get_forgotten(selected_profile))
  ...
  for (l = profiles; l != NULL; l = l->next) {
    ...
    gtk_list_store_insert_with_values(store, &iter, 0, (int)COL_PROFILE,
                                      profile, (int)-1);
  ...
  /* Now turn on sorting */
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(store), COL_PROFILE,
                                  terminal_app_profile_sort_func, NULL, NULL);
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store), COL_PROFILE,
                                       GTK_SORT_ASCENDING);

  return GTK_TREE_MODEL(store);
}

static /* ref */ TerminalProfile *profile_combo_box_get_selected(
    GtkWidget *widget) {
  ...
  if (gtk_combo_box_get_active_iter(combo, &iter))
    gtk_tree_model_get(gtk_combo_box_get_model(combo), &iter, (int)COL_PROFILE,
                       &profile, (int)-1);

  return profile;
}

static void profile_combo_box_refill(TerminalApp *app, GtkWidget *widget) {
  GtkComboBox *combo = GTK_COMBO_BOX(widget);
  GtkTreeIter iter;
  gboolean iter_set;
  ...
  if (!selected_profile) {
    ...
    if (selected_profile) g_object_ref(selected_profile);
  }

  model = terminal_app_get_profile_liststore(app, selected_profile, &iter,
                                             &iter_set);
  gtk_combo_box_set_model(combo, model);
  g_object_unref(model);

  if (iter_set) gtk_combo_box_set_active_iter(combo, &iter);

  if (selected_profile) g_object_unref(selected_profile);
}

static GtkWidget *profile_combo_box_new(TerminalApp *app) {
  GtkWidget *combo;
  GtkCellRenderer *renderer;

  combo = gtk_combo_box_new();
  terminal_util_set_atk_name_description(combo, NULL,
                                         _("Click button to choose profile"));

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combo), renderer, TRUE);
  gtk_cell_layout_set_cell_data_func(
      GTK_CELL_LAYOUT(combo), renderer,
      (GtkCellLayoutDataFunc)terminal_app_profile_cell_data_func, NULL, NULL);
...
static void profile_combo_box_changed_cb(GtkWidget *widget, TerminalApp *app) {
  TerminalProfile *profile;
  ...
  g_settings_set_string(
      settings_global, DEFAULT_PROFILE_KEY,
      terminal_profile_get_property_string(profile, TERMINAL_PROFILE_NAME));

  /* Even though the GSettings change notification does this, it happens too
   * late. In some cases, the default profile changes twice in quick succession,
...
static void profile_list_treeview_refill(TerminalApp *app, GtkWidget *widget) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter))
    gtk_tree_model_get(model, &iter, (int)COL_PROFILE, &selected_profile,
                       (int)-1);

  model = terminal_app_get_profile_liststore(
      terminal_app_get(), selected_profile, &iter, &iter_set);
  gtk_tree_view_set_model(tree_view, model);
  g_object_unref(model);

  if (!iter_set) iter_set = gtk_tree_model_get_iter_first(model, &iter);

  if (iter_set) gtk_tree_selection_select_iter(selection, &iter);

  if (selected_profile) g_object_unref(selected_profile);
}

static GtkWidget *profile_list_treeview_create(TerminalApp *app) {
  GtkWidget *tree_view;
  GtkTreeSelection *selection;
  GtkCellRenderer *renderer;
...
static void profile_list_delete_confirm_response_cb(GtkWidget *dialog,
                                                    int response) {
  TerminalProfile *profile;
  ...
  if (response == GTK_RESPONSE_ACCEPT) terminal_app_delete_profile(profile);

  gtk_widget_destroy(dialog);
}

static void mate_dialog_add_button(GtkDialog *dialog, const gchar *button_text,
...
static void profile_list_delete_button_clicked_cb(GtkWidget *button,
                                                  GtkWidget *widget) {
  GtkTreeView *tree_view = GTK_TREE_VIEW(widget);
  GtkTreeSelection *selection;
  GtkWidget *dialog;
  ...
  dialog = gtk_message_dialog_new(
      GTK_WINDOW(transient_parent), GTK_DIALOG_DESTROY_WITH_PARENT,
      GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE, _("Delete profile “%s”?"),
      terminal_profile_get_property_string(selected_profile,
                                           TERMINAL_PROFILE_VISIBLE_NAME));

  mate_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), "process-stop",
                         GTK_RESPONSE_REJECT);

  mate_dialog_add_button(GTK_DIALOG(dialog), _("_Delete"), "edit-delete",
                         GTK_RESPONSE_ACCEPT);
  ...
  g_object_set_data_full(G_OBJECT(dialog), "profile", selected_profile,
                         g_object_unref);

  g_signal_connect(dialog, "response",
                   G_CALLBACK(profile_list_delete_confirm_response_cb), NULL);

  gtk_window_present(GTK_WINDOW(dialog));
}

static void profile_list_new_button_clicked_cb(GtkWidget *button,
                                               gpointer data) {
  TerminalApp *app;
...
static void profile_list_edit_button_clicked_cb(GtkWidget *button,
                                                GtkWidget *widget) {
  GtkTreeView *tree_view = GTK_TREE_VIEW(widget);
  GtkTreeSelection *selection;
  GtkTreeIter iter;
  ...
  terminal_app_edit_profile(app, selected_profile,
                            GTK_WINDOW(app->manage_profiles_dialog), NULL);
  g_object_unref(selected_profile);
}

static void profile_list_row_activated_cb(GtkTreeView *tree_view,
                                          GtkTreePath *path,
                                          GtkTreeViewColumn *column,
                                          gpointer data) {
  GtkTreeIter iter;
...
static GList *find_profile_link(GList *profiles, const char *name) {
  GList *l;

  for (l = profiles; l != NULL; l = l->next) {
    const char *profile_name;

    profile_name = terminal_profile_get_property_string(
        TERMINAL_PROFILE(l->data), TERMINAL_PROFILE_NAME);
    if (profile_name && strcmp(profile_name, name) == 0) break;
  }

  return l;
}

static void terminal_app_profile_list_notify_cb(GSettings *settings,
                                                const gchar *key,
                                                gpointer user_data) {
...
ensure_one_profile:
  ...
  if (val != NULL) g_variant_unref(val);
  ...
  for (l = profiles_to_delete; l != NULL; l = l->next) {
    ...
    if (strcmp(name, FALLBACK_PROFILE_ID) == 0) continue;
    ...
    /* |profile| possibly isn't dead yet since the profiles dialogue's tree
     * model holds a ref too... */
  }
...
static void terminal_app_default_profile_notify_cb(GSettings *settings,
                                                   const gchar *key,
                                                   gpointer user_data) {
  TerminalApp *app = TERMINAL_APP(user_data);
  GVariant *val;
  ...
  if (!name || !name[0]) name = FALLBACK_PROFILE_ID;
  g_assert(name != NULL);
...
static int compare_encodings(TerminalEncoding *a, TerminalEncoding *b) {
  return g_utf8_collate(a->name, b->name);
}

static void encoding_mark_active(gpointer key, gpointer value, gpointer data) {
...
static void terminal_app_encoding_list_notify_cb(GSettings *settings,
                                                 const gchar *key,
                                                 gpointer user_data) {
  TerminalApp *app = TERMINAL_APP(user_data);
  GVariant *val;
  ...
  g_hash_table_foreach(app->encodings, (GHFunc)encoding_mark_active,
                       GUINT_TO_POINTER(FALSE));

  /* First add the locale's charset */
  encoding = g_hash_table_lookup(app->encodings, "current");
  g_assert(encoding);
  if (terminal_encoding_is_valid(encoding)) encoding->is_active = TRUE;
...
static void terminal_app_system_font_notify_cb(GSettings *settings,
                                               const gchar *key,
                                               gpointer user_data) {
  TerminalApp *app = TERMINAL_APP(user_data);
  GVariant *val;
  ...
  if (app->system_font_desc) pango_font_description_free(app->system_font_desc);
...
static void terminal_app_enable_mnemonics_notify_cb(GSettings *settings,
                                                    const gchar *key,
                                                    gpointer user_data) {
  TerminalApp *app = TERMINAL_APP(user_data);
  gboolean enable;
  ...
  if (enable == app->enable_mnemonics) return;

  app->enable_mnemonics = enable;
  g_object_notify(G_OBJECT(app), TERMINAL_APP_ENABLE_MNEMONICS);
}

static void terminal_app_enable_menu_accels_notify_cb(GSettings *settings,
                                                      const gchar *key,
                                                      gpointer user_data) {
  TerminalApp *app = TERMINAL_APP(user_data);
  gboolean enable;
  ...
  enable = g_settings_get_boolean(settings, key);
  if (enable == app->enable_menu_accels) return;

  app->enable_menu_accels = enable;
  g_object_notify(G_OBJECT(app), TERMINAL_APP_ENABLE_MENU_BAR_ACCEL);
}

static void new_profile_response_cb(GtkWidget *new_profile_dialog,
                                    int response_id, TerminalApp *app) {
  if (response_id == GTK_RESPONSE_ACCEPT) {
    ...
    base_option_menu =
        g_object_get_data(G_OBJECT(new_profile_dialog), "base_option_menu");
    base_profile = profile_combo_box_get_selected(base_option_menu);
    if (!base_profile) base_profile = terminal_app_get_default_profile(app);
    if (!base_profile) return; /* shouldn't happen ever though */
    ...
    for (tmp = profiles; tmp != NULL; tmp = tmp->next) {
      ...
      if (visible_name && strcmp(name, visible_name) == 0) break;
    }
    if (tmp) {
      confirm_dialog = gtk_message_dialog_new(
          GTK_WINDOW(new_profile_dialog), GTK_DIALOG_DESTROY_WITH_PARENT,
          GTK_MESSAGE_QUESTION, GTK_BUTTONS_YES_NO,
          _("You already have a profile called “%s”. Do you want to create "
            "another profile with the same name?"),
          name);
      /* Alternative button order was set automatically by GtkMessageDialog */
      retval = gtk_dialog_run(GTK_DIALOG(confirm_dialog));
      gtk_widget_destroy(confirm_dialog);
      if (retval == GTK_RESPONSE_NO) goto cleanup;
    }
    g_list_free(profiles);
    ...
    new_profile_name = terminal_profile_get_property_string(
        new_profile, TERMINAL_PROFILE_NAME);
    g_hash_table_insert(app->profiles, g_strdup(new_profile_name),
                        new_profile /* adopts the refcount */);

    /* And now save the new profile name to GSettings */
    gsettings_append_strv(settings_global, PROFILE_LIST_KEY, new_profile_name);
...
static void new_profile_dialog_destroy_cb(GtkWidget *new_profile_dialog,
                                          TerminalApp *app) {
  GtkWidget *combo;

  combo = g_object_get_data(G_OBJECT(new_profile_dialog), "base_option_menu");
  g_signal_handlers_disconnect_by_func(
      app, G_CALLBACK(profile_combo_box_refill), combo);

  app->new_profile_dialog = NULL;
}

static void new_profile_name_entry_changed_cb(GtkEntry *entry,
                                              GtkDialog *dialog) {
  const char *name;

  name = gtk_entry_get_text(entry);

  /* make the create button sensitive only if something other than space has
   * been set */
  while (*name != '\0' && g_ascii_isspace(*name)) ++name;
...
void terminal_app_new_profile(TerminalApp *app,
  ...
  if (app->new_profile_dialog == NULL) {
    GtkWidget *create_button, *grid, *name_label, *name_entry, *base_label,
        *combo;

    if (!terminal_util_load_builder_resource(
            TERMINAL_RESOURCES_PATH_PREFIX G_DIR_SEPARATOR_S
            "ui/profile-new-dialog.ui",
            "new-profile-dialog", &app->new_profile_dialog,
            "new-profile-create-button", &create_button, "new-profile-grid",
            &grid, "new-profile-name-label", &name_label,
            "new-profile-name-entry", &name_entry, "new-profile-base-label",
            &base_label, NULL))
      return;
    ...
    g_object_set_data(G_OBJECT(app->new_profile_dialog), "create_button",
                      create_button);
    gtk_widget_set_sensitive(create_button, FALSE);

    /* the name entry */
    g_object_set_data(G_OBJECT(app->new_profile_dialog), "name_entry",
                      name_entry);
    g_signal_connect(name_entry, "changed",
                     G_CALLBACK(new_profile_name_entry_changed_cb),
    ...
    g_object_set_data(G_OBJECT(app->new_profile_dialog), "base_option_menu",
                      combo);
    terminal_util_set_atk_name_description(combo, NULL,
                                           _("Choose base profile"));
    ...
    gtk_dialog_set_default_response(GTK_DIALOG(app->new_profile_dialog),
                                    GTK_RESPONSE_ACCEPT);
    gtk_dialog_set_response_sensitive(GTK_DIALOG(app->new_profile_dialog),
...
static void profile_list_selection_changed_cb(GtkTreeSelection *selection,
                                              TerminalApp *app) {
  gboolean selected;
  ...
  gtk_widget_set_sensitive(app->manage_profiles_delete_button,
                           selected && g_hash_table_size(app->profiles) > 1);
}

static void profile_list_response_cb(GtkWidget *dialog, int id,
                                     TerminalApp *app) {
  g_assert(app->manage_profiles_dialog == dialog);

  if (id == GTK_RESPONSE_HELP) {
    terminal_util_show_help("mate-terminal-manage-profiles",
                            GTK_WINDOW(dialog));
    return;
  }
...
static void profile_list_destroyed_cb(GtkWidget *manage_profiles_dialog,
                                      TerminalApp *app) {
  g_signal_handlers_disconnect_by_func(
      app, G_CALLBACK(profile_list_treeview_refill), app->manage_profiles_list);
  g_signal_handlers_disconnect_by_func(app,
                                       G_CALLBACK(profile_combo_box_refill),
...
void terminal_app_manage_profiles(TerminalApp *app,
  ...
  if (!terminal_util_load_builder_resource(
          TERMINAL_RESOURCES_PATH_PREFIX G_DIR_SEPARATOR_S
          "ui/profile-manager.ui",
          "profile-manager", &dialog, "profiles-treeview-container",
          &tree_view_container, "new-profile-button", &new_button,
          "edit-profile-button", &edit_button, "delete-profile-button",
          &remove_button, "default-profile-hbox", &default_hbox,
          "default-profile-label", &default_label, NULL))
    return;
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(profile_list_response_cb),
                   app);
  g_signal_connect(dialog, "destroy", G_CALLBACK(profile_list_destroyed_cb),
                   app);

  app->manage_profiles_list = profile_list_treeview_create(app);

  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(app->manage_profiles_list));
  g_signal_connect(selection, "changed",
                   G_CALLBACK(profile_list_selection_changed_cb), app);

  profile_list_treeview_refill(app, app->manage_profiles_list);
  g_signal_connect(app, "profile-list-changed",
                   G_CALLBACK(profile_list_treeview_refill),
  ...
  gtk_container_add(GTK_CONTAINER(tree_view_container),
                    app->manage_profiles_list);
...
#ifdef HAVE_SMCLIENT
static void terminal_app_save_state_cb(EggSMClient *client, GKeyFile *key_file,
                                       TerminalApp *app) {
  terminal_app_save_config(app, key_file);
}

static void terminal_app_client_quit_cb(EggSMClient *client, TerminalApp *app) {
...
static void terminal_app_init(TerminalApp *app) {
  ...
  g_signal_connect(settings_global, "changed::" PROFILE_LIST_KEY,
                   G_CALLBACK(terminal_app_profile_list_notify_cb), app);

  g_signal_connect(settings_global, "changed::" DEFAULT_PROFILE_KEY,
                   G_CALLBACK(terminal_app_default_profile_notify_cb), app);

  g_signal_connect(settings_global, "changed::" ENCODING_LIST_KEY,
                   G_CALLBACK(terminal_app_encoding_list_notify_cb), app);

  g_signal_connect(app->settings_font, "changed::" MONOSPACE_FONT_KEY,
                   G_CALLBACK(terminal_app_system_font_notify_cb), app);

  g_signal_connect(settings_global, "changed::" ENABLE_MNEMONICS_KEY,
                   G_CALLBACK(terminal_app_enable_mnemonics_notify_cb), app);

  g_signal_connect(settings_global, "changed::" ENABLE_MENU_BAR_ACCEL_KEY,
                   G_CALLBACK(terminal_app_enable_menu_accels_notify_cb), app);

  /* Load the settings */
  terminal_app_profile_list_notify_cb(settings_global, PROFILE_LIST_KEY, app);
  terminal_app_default_profile_notify_cb(settings_global, DEFAULT_PROFILE_KEY,
                                         app);
  terminal_app_encoding_list_notify_cb(settings_global, ENCODING_LIST_KEY, app);
  ...
  terminal_app_enable_menu_accels_notify_cb(settings_global,
                                            ENABLE_MENU_BAR_ACCEL_KEY, app);
  terminal_app_enable_mnemonics_notify_cb(settings_global, ENABLE_MNEMONICS_KEY,
                                          app);
  ...
  desktop_file =
      g_build_filename(TERM_DATADIR, "applications", PACKAGE ".desktop", NULL);
  ...
  g_signal_connect(sm_client, "quit", G_CALLBACK(terminal_app_client_quit_cb),
                   app);
#endif /* HAVE_SMCLIENT */
}
...
static void terminal_app_finalize(GObject *object) {
  ...
  sm_client = egg_sm_client_get();
  g_signal_handlers_disconnect_matched(sm_client, G_SIGNAL_MATCH_DATA, 0, 0,
                                       NULL, NULL, app);
#endif /* HAVE_SMCLIENT */

  g_signal_handlers_disconnect_by_func(
      settings_global, G_CALLBACK(terminal_app_profile_list_notify_cb), app);
  g_signal_handlers_disconnect_by_func(
      settings_global, G_CALLBACK(terminal_app_default_profile_notify_cb), app);
  g_signal_handlers_disconnect_by_func(
      settings_global, G_CALLBACK(terminal_app_encoding_list_notify_cb), app);
  g_signal_handlers_disconnect_by_func(
      app->settings_font, G_CALLBACK(terminal_app_system_font_notify_cb), app);
  g_signal_handlers_disconnect_by_func(
      settings_global, G_CALLBACK(terminal_app_enable_menu_accels_notify_cb),
      app);
  g_signal_handlers_disconnect_by_func(
      settings_global, G_CALLBACK(terminal_app_enable_mnemonics_notify_cb),
      app);
...
static void terminal_app_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_SYSTEM_FONT:
      if (app->system_font_desc)
        ...
        g_value_take_boxed(
            value, pango_font_description_from_string(DEFAULT_MONOSPACE_FONT));
      break;
...
static void terminal_app_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_ENABLE_MENU_BAR_ACCEL:
      ...
      g_settings_set_boolean(settings_global, ENABLE_MENU_BAR_ACCEL_KEY,
                             app->enable_menu_accels);
      break;
    case PROP_ENABLE_MNEMONICS:
      app->enable_mnemonics = g_value_get_boolean(value);
      g_settings_set_boolean(settings_global, ENABLE_MNEMONICS_KEY,
                             app->enable_mnemonics);
      break;
    case PROP_DEFAULT_PROFILE:
    case PROP_SYSTEM_FONT:
      /* not writable */
...
static void terminal_app_real_quit(TerminalApp *app) { gtk_main_quit(); }
...
static void terminal_app_class_init(TerminalAppClass *klass) {
  ...
  signals[QUIT] =
      g_signal_new(I_("quit"), G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(TerminalAppClass, quit),
  ...
  g_object_class_install_property(
      object_class, PROP_ENABLE_MENU_BAR_ACCEL,
  ...
  g_object_class_install_property(
      object_class, PROP_ENABLE_MNEMONICS,
  ...
  g_object_class_install_property(
      object_class, PROP_DEFAULT_PROFILE,
 ...
 *   error
 */
gboolean terminal_app_handle_options(TerminalApp *app, TerminalOptions *options,
                                     gboolean allow_resume, GError **error) {
  GList *lw;
  ...
  if (options->save_config) {
    ...
    g_set_error_literal(
        error, TERMINAL_OPTION_ERROR, TERMINAL_OPTION_ERROR_NOT_IN_FACTORY,
  ...
  if (options->load_config) {
    ...
    result =
        g_key_file_load_from_file(key_file, options->config_file, 0, error) &&
  ...
  if (allow_resume && egg_sm_client_is_resumed(sm_client)) {
    ...
    if (key_file != NULL && !terminal_options_merge_config(
                                options, key_file, SOURCE_SESSION, error))
      return FALSE;
  }
#endif /* HAVE_SMCLIENT */
  ...
  if (options->startup_id != NULL)
    _terminal_debug_print(TERMINAL_DEBUG_FACTORY, "Startup ID is '%s'\n",
                          options->startup_id);
  ...
  for (lw = options->initial_windows; lw != NULL; lw = lw->next) {
    ...
    if (lw == options->initial_windows &&
        ((InitialTab *)iw->tabs->data)->attach_window)
      window = terminal_app_get_current_window(app, gdk_screen,
    ...
    if (!window) {
       ...
       */
      if (iw->role) gtk_window_set_role(GTK_WINDOW(window), iw->role);

      if (iw->force_menubar_state)
        terminal_window_set_menubar_visible(window, iw->menubar_state);

      if (iw->start_fullscreen) gtk_window_fullscreen(GTK_WINDOW(window));
    ...
    for (lt = iw->tabs; lt != NULL; lt = lt->next) {
      ...
      if (profile == NULL) profile = terminal_app_get_profile_for_new_term(app);
      g_assert(profile);

      screen = terminal_app_new_terminal(
          app, window, profile,
          ...
          options->env, it->zoom_set ? it->zoom : options->zoom);

      if (it->active) terminal_window_switch_screen(window, screen);
    }
    ...
    if (iw->geometry) {
      ...
      if (!terminal_window_update_size_set_geometry(
              window, terminal_window_get_active(window), FALSE, iw->geometry))
...
TerminalWindow *terminal_app_new_window(TerminalApp *app, GdkScreen *screen) {
  TerminalWindow *window;

  window = terminal_window_new();

  app->windows = g_list_append(app->windows, window);
  g_signal_connect(window, "destroy", G_CALLBACK(terminal_window_destroyed),
                   app);

  if (screen) gtk_window_set_screen(GTK_WINDOW(window), screen);

  return window;
}

TerminalScreen *terminal_app_new_terminal(
    TerminalApp *app, TerminalWindow *window, TerminalProfile *profile,
    char **override_command, const char *title, const char *working_dir,
    char **child_env, double zoom) {
  TerminalScreen *screen;

  g_return_val_if_fail(TERMINAL_IS_APP(app), NULL);
  g_return_val_if_fail(TERMINAL_IS_WINDOW(window), NULL);

  screen = terminal_screen_new(profile, override_command, title, working_dir,
                               child_env, zoom);
...
void terminal_app_edit_profile(TerminalApp *app, TerminalProfile *profile,
                               GtkWindow *transient_parent,
...
TerminalWindow *terminal_app_get_current_window(TerminalApp *app,
  ...
  if (app->windows == NULL) return NULL;
  ...
  while (res) {
    int win_workspace;
    if (gtk_window_get_screen(GTK_WINDOW(res->data)) != from_screen) continue;
...
GList *terminal_app_get_profile_list(TerminalApp *app) {
  ...
  return g_list_sort(g_hash_table_get_values(app->profiles),
                     profiles_alphabetic_cmp);
}
...
TerminalProfile *terminal_app_get_profile_by_visible_name(TerminalApp *app,
  ...
  g_hash_table_foreach(app->profiles, profiles_lookup_by_visible_name_foreach,
                       &info);
  return info.result;
}
...
TerminalProfile *terminal_app_get_profile_for_new_term(TerminalApp *app) {
  ...
  if (app->default_profile) return app->default_profile;
...
TerminalEncoding *terminal_app_ensure_encoding(TerminalApp *app,
  ...
  if (encoding == NULL) {
    encoding = terminal_encoding_new(charset, _("User Defined"), TRUE,
                                     TRUE /* scary! */);
    g_hash_table_insert(app->encodings,
                        (gpointer)terminal_encoding_get_id(encoding), encoding);
  }
 ...
 *
 * Returns: a newly allocated list of newly referenced #TerminalEncoding
 * objects.
 */
GSList *terminal_app_get_active_encodings(TerminalApp *app) {
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    if (!encoding->is_active) continue;
...
void terminal_app_save_config(TerminalApp *app, GKeyFile *key_file) {
  GList *lw;
  guint n = 0;
  ...
  g_key_file_set_comment(key_file, NULL, NULL, "Written by " PACKAGE_STRING,
                         NULL);

  g_key_file_set_integer(key_file, TERMINAL_CONFIG_GROUP,
                         TERMINAL_CONFIG_PROP_VERSION, TERMINAL_CONFIG_VERSION);
  g_key_file_set_integer(key_file, TERMINAL_CONFIG_GROUP,
                         TERMINAL_CONFIG_PROP_COMPAT_VERSION,
                         TERMINAL_CONFIG_COMPAT_VERSION);
  ...
  g_key_file_set_string_list(key_file, TERMINAL_CONFIG_GROUP,
                             TERMINAL_CONFIG_PROP_WINDOWS,
                             (const char *const *)window_names, len);
  g_strfreev(window_names);
}

gboolean terminal_app_save_config_file(TerminalApp *app, const char *file_name,
                                       GError **error) {
  GKeyFile *key_file;
  char *data;
  gsize len;
