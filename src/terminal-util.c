...
void terminal_util_set_unique_role(GtkWindow *window, const char *prefix) {
  ...
  role = g_strdup_printf("%s-%d-%d-%" G_GINT64_FORMAT, prefix, getpid(),
                         g_random_int(), g_get_real_time() / G_USEC_PER_SEC);
 ...
 *
 * Create a #GtkMessageDialog window with the message, and present it, handling
 * its buttons. If @weap_ptr is not #NULL, only create the dialog if
 * <literal>*weap_ptr</literal> is #NULL (and in that * case, set @weap_ptr to
 * be a weak pointer to the new dialog), otherwise just present
 * <literal>*weak_ptr</literal>. Note that in this last case, the message
 * <emph>will</emph> be changed.
 */
void terminal_util_show_error_dialog(GtkWindow *transient_parent,
  ...
  if (weak_ptr == NULL || *weak_ptr == NULL) {
    ...
    dialog = gtk_message_dialog_new(
        transient_parent, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
        GTK_BUTTONS_OK, message ? "%s" : NULL, message);
...
void terminal_util_show_help(const char *topic, GtkWindow *parent) {
  GError *error = NULL;
  char *url;
...
void terminal_util_set_atk_name_description(GtkWidget *widget, const char *name,
  ...
  if (obj == NULL) {
    g_warning("%s: for some reason widget has no GtkAccessible", G_STRFUNC);
    return;
  }
  ...
  if (desc) atk_object_set_description(obj, desc);
  if (name) atk_object_set_name(obj, name);
}
...
void terminal_util_open_url(GtkWidget *parent, const char *orig_url,
  ...
  if (!gtk_show_uri_on_window(GTK_WINDOW(parent), uri, user_time, &error)) {
    terminal_util_show_error_dialog(GTK_WINDOW(parent), NULL, error,
                                    _("Could not open the address “%s”"), uri);
...
char *terminal_util_resolve_relative_path(const char *path,
  ...
  if (path == NULL) return g_strdup(relative_path);

  file = g_file_new_for_path(path);
  resolved_file = g_file_resolve_relative_path(file, relative_path);
  g_object_unref(file);

  if (resolved_file == NULL) return NULL;
...
char *terminal_util_concat_uris(char **uris, gsize *length) {
  ...
  len = 0;
  for (i = 0; uris[i]; ++i) len += strlen(uris[i]) + 1;
...
char *terminal_util_get_licence_text(void) {
  const gchar *license[] = {
      N_("MATE Terminal is free software; you can redistribute it and/or "
         "modify "
         "it under the terms of the GNU General Public License as published by "
         "the Free Software Foundation; either version 3 of the License, or "
         "(at your option) any later version."),
      ...
      N_("You should have received a copy of the GNU General Public License "
         "along with MATE Terminal; if not, write to the Free Software "
         "Foundation, "
...
void terminal_util_key_file_set_argv(GKeyFile *key_file, const char *group,
  ...
  if (argc < 0) argc = g_strv_length(argv);

  quoted_argv = g_new(char *, argc + 1);
  for (i = 0; i < argc; ++i) quoted_argv[i] = g_shell_quote(argv[i]);
...
char **terminal_util_key_file_get_argv(GKeyFile *key_file, const char *group,
  ...
  flat =
      terminal_util_key_file_get_string_unescape(key_file, group, key, error);
  if (!flat) return NULL;
...
static char *gsettings_get_string(GSettings *settings, const char *key) {
  char *value;
 ...
 */
static void set_proxy_env(GHashTable *env_table, const char *key, char *value) {
  char *key1 = NULL, *key2 = NULL;
  char *value1 = NULL, *value2 = NULL;

  if (!value) return;

  if (g_hash_table_lookup(env_table, key) == NULL) key1 = g_strdup(key);
  ...
  if (key1) g_hash_table_replace(env_table, key1, value1);
  if (key2) g_hash_table_replace(env_table, key2, value2);
}

static void setup_http_proxy_env(GHashTable *env_table,
...
static void setup_ignore_host_env(GHashTable *env_table, GSettings *settings) {
...
static void setup_https_proxy_env(GHashTable *env_table,
...
static void setup_ftp_proxy_env(GHashTable *env_table,
...
static void setup_socks_proxy_env(GHashTable *env_table,
...
static void setup_autoconfig_proxy_env(GHashTable *env_table,
                                       GSettings *settings) {
...
static gboolean transform_boolean(gboolean input, PropertyChangeFlags flags) {
  if (flags & FLAG_INVERT_BOOL) input = !input;

  return input;
}

static void object_change_notify_cb(PropertyChange *change) {
  ...
  if (GTK_IS_RADIO_BUTTON(widget)) {
    ...
    if (name) filename = g_filename_from_utf8(name, -1, NULL, NULL, NULL);
...
static void widget_change_notify_cb(PropertyChange *change) {
  GObject *object = change->object;
  const char *object_prop = change->object_prop;
  GtkWidget *widget = change->widget;
  ...
  if (GTK_IS_RADIO_BUTTON(widget)) {
    ...
    g_object_set(object, object_prop, transform_boolean(enabled, change->flags),
                 NULL);
  } else if (GTK_IS_SPIN_BUTTON(widget)) {
    gint value;
    ...
    if (filename) name = g_filename_to_utf8(filename, -1, NULL, NULL, NULL);
...
void terminal_util_bind_object_property_to_widget(GObject *object,
  ...
  g_object_set_data_full(G_OBJECT(widget), "GT:PCD", change,
                         (GDestroyNotify)property_change_free);
  ...
  change->widget_notify_id = g_signal_connect_swapped(
      widget, signal_name, G_CALLBACK(widget_change_notify_cb), change);

  change->object = object;
  change->flags = flags;
  change->object_prop = object_prop;

  g_snprintf(notify_signal_name, sizeof(notify_signal_name), "notify::%s",
             object_prop);
  object_change_notify_cb(change);
  change->object_notify_id = g_signal_connect_swapped(
      object, notify_signal_name, G_CALLBACK(object_change_notify_cb), change);
}

#ifdef GDK_WINDOWING_X11

/* Asks the window manager to turn off the "demands attention" state on the
 * window.
 *
 * This only works for windows that are currently window managed; if the window
 * is unmapped (in the withdrawn state) it would be necessary to change
 * _NET_WM_STATE directly.
 */
void terminal_util_x11_clear_demands_attention(GdkWindow *window) {
  ...
  xclient.data.l[1] = gdk_x11_get_xatom_by_name_for_display(
      display, "_NET_WM_STATE_DEMANDS_ATTENTION");
  xclient.data.l[2] = 0;
  xclient.data.l[3] = 0;
  xclient.data.l[4] = 0;

  XSendEvent(GDK_DISPLAY_XDISPLAY(display),
             GDK_WINDOW_XID(gdk_screen_get_root_window(screen)), False,
