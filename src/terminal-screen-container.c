...
G_DEFINE_TYPE_WITH_PRIVATE(TerminalScreenContainer, terminal_screen_container,
                           GTK_TYPE_BOX)

/* helper functions */

static void terminal_screen_container_set_placement_internal(
    TerminalScreenContainer *container, GtkCornerType corner) {
  TerminalScreenContainerPrivate *priv = container->priv;

#ifdef USE_SCROLLED_WINDOW
  gtk_scrolled_window_set_placement(GTK_SCROLLED_WINDOW(priv->scrolled_window),
                                    corner);
#else
  switch (corner) {
...
static void terminal_screen_container_set_placement_set(
    TerminalScreenContainer *container, gboolean set) {
  TerminalScreenContainerPrivate *priv = container->priv;
...
static void terminal_screen_container_init(TerminalScreenContainer *container) {
  TerminalScreenContainerPrivate *priv;

  priv = container->priv =
      terminal_screen_container_get_instance_private(container);
...
static GObject *terminal_screen_container_constructor(
  ...
  object = G_OBJECT_CLASS(terminal_screen_container_parent_class)
  ...
  priv->scrolled_window = gtk_scrolled_window_new(
      NULL, gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(priv->screen)));

  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(priv->scrolled_window),
                                 priv->hscrollbar_policy,
                                 priv->vscrollbar_policy);
  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(priv->scrolled_window), GTK_SHADOW_NONE);
  gtk_container_add(GTK_CONTAINER(priv->scrolled_window),
                    GTK_WIDGET(priv->screen));
  gtk_widget_show(GTK_WIDGET(priv->screen));
  gtk_box_pack_end(GTK_BOX(container), priv->scrolled_window, TRUE, TRUE, 0);
  gtk_widget_show(priv->scrolled_window);

#ifdef MATE_ENABLE_DEBUG
  g_signal_connect(priv->scrolled_window, "size-allocate",
                   G_CALLBACK(size_allocate_cb), container);
#endif
  ...
  priv->vscrollbar = gtk_scrollbar_new(
      GTK_ORIENTATION_VERTICAL,
      gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(priv->screen)));

  gtk_box_pack_start(GTK_BOX(priv->hbox), GTK_WIDGET(priv->screen), TRUE, TRUE,
                     0);
...
static void terminal_screen_container_get_property(GObject *object,
...
static void terminal_screen_container_set_property(GObject *object,
                                                   guint prop_id,
                                                   const GValue *value,
                                                   GParamSpec *pspec) {
  TerminalScreenContainer *container = TERMINAL_SCREEN_CONTAINER(object);
  ...
  switch (prop_id) {
    case PROP_SCREEN:
      ...
      break;
    case PROP_HSCROLLBAR_POLICY:
      terminal_screen_container_set_policy(container, g_value_get_enum(value),
                                           priv->vscrollbar_policy);
      break;
    case PROP_VSCROLLBAR_POLICY:
      terminal_screen_container_set_policy(container, priv->hscrollbar_policy,
                                           g_value_get_enum(value));
      break;
    case PROP_WINDOW_PLACEMENT:
      terminal_screen_container_set_placement_internal(container,
                                                       g_value_get_enum(value));
      break;
    case PROP_WINDOW_PLACEMENT_SET:
      terminal_screen_container_set_placement_set(container,
...
static void terminal_screen_container_class_init(
    TerminalScreenContainerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "screen", NULL, NULL, TERMINAL_TYPE_SCREEN,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      gobject_class, PROP_HSCROLLBAR_POLICY,
      g_param_spec_enum("hscrollbar-policy", NULL, NULL, GTK_TYPE_POLICY_TYPE,
                        GTK_POLICY_AUTOMATIC,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property(
      gobject_class, PROP_VSCROLLBAR_POLICY,
      g_param_spec_enum("vscrollbar-policy", NULL, NULL, GTK_TYPE_POLICY_TYPE,
                        GTK_POLICY_AUTOMATIC,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      gobject_class, PROP_WINDOW_PLACEMENT,
      g_param_spec_enum("window-placement", NULL, NULL, GTK_TYPE_CORNER_TYPE,
                        GTK_CORNER_TOP_LEFT,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      gobject_class, PROP_WINDOW_PLACEMENT_SET,
      g_param_spec_boolean("window-placement-set", NULL, NULL, FALSE,
                           G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
GtkWidget *terminal_screen_container_new(TerminalScreen *screen) {
  return g_object_new(TERMINAL_TYPE_SCREEN_CONTAINER, "screen", screen, NULL);
}
 ...
 */
TerminalScreen *terminal_screen_container_get_screen(
    TerminalScreenContainer *container) {
 ...
 */
TerminalScreenContainer *terminal_screen_container_get_from_screen(
    TerminalScreen *screen) {
  g_return_val_if_fail(TERMINAL_IS_SCREEN(screen), NULL);

  return TERMINAL_SCREEN_CONTAINER(gtk_widget_get_ancestor(
      GTK_WIDGET(screen), TERMINAL_TYPE_SCREEN_CONTAINER));
}
